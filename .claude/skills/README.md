...
| `checkpoint` | Checkpoint session context to a tracking bead. Ensures ONE bead captures all active work, recent commits, uncommitted changes, and next steps. Use before /compact, at natural breakpoints, or when context is getting long. Also runs automatically via pre-compact hook. | `checkpoint`, `compact`, `resume`, `context`, `bead` |
| `claim` | Claim an @agent/N persona slot. Sets assignee + status=wip via DB-side CAS, reads slot body into session context as <persona> envelope, broadcasts on tribe. Optional trailing bead IDs queue-assign those beads to the slot. Required before /do. | `claim`, `agent`, `persona`, `slot`, `assign`, `lock`, `queue` |
...
| `commit` | Commit changes to git. Use when ready to commit staged or unstaged changes. | `commit`, `git commit`, `staged`, `unstaged` |
...
| `diagram` | ASCII diagram creation — aligned boxes, trees, flow diagrams. Use when creating diagrams in markdown docs. Prevents the chronic misalignment bug. | `diagram`, `ascii`, `flow`, `boxes`, `tree` |
| `discuss` | Pause implementation to discuss architecture, alternatives, or understanding. Use when you want to step back and discuss before coding. Checkpoints context to active bead for safe resumption. | `discuss`, `pause`, `architecture`, `alternatives` |
| `do` | Pick and work the highest-priority bead from any @agent/N slot you've claimed, OR work specific beads passed as args. Composes with /loop for continuous mode. Tribe-broadcasts on bead-pick and bead-close. | `do`, `work`, `next`, `queue`, `agent`, `persona`, `dispatch`, `bead` |
...
| `eli5` | Re-explain to a teammate who just walked in. Strips task-shorthand, names mechanisms, caps at one screen. Use when the user signals they're lost ("explain", "wait what?", "I don't have your context", "ELI5"), or when you catch yourself using terms specific to the current task (option names, numbered suspects, file paths without gloss). | `eli5`, `explain`, `plain`, `lost`, `context-drift`, `jargon` |
| `explore` | TUI exploration - interactive AI probing + targeted testing + randomized bug hunting. Use when exercising km view to find bugs, test scenarios, or inspect the live terminal. | `explore`, `tui`, `bug hunting`, `terminal`, `randomized` |
| `flexily` | Debug and fix Flexily layout issues — caching, fingerprinting, zero-allocation, performance. Use when Flexily layout is broken or performance degrades. | `flexily`, `layout`, `flexbox`, `measure`, `performance` |
| `fp-check` | Deprecated — false positive checking is done inline during reviews. | `false positive`, `fp-check`, `review` |
...
| `ink-compat` | Silvery vs Ink comparison, compat upgrade, benchmarking, and positioning analysis. One skill for all Ink-related work. Use when Ink releases a new version, when updating silvery-vs-ink docs, when benchmarking, or when planning silvery's positioning. | `ink`, `compatibility`, `benchmark`, `positioning`, `silvery` |
| `lmstudio` | Query local LM Studio server on :1234; auto-starts via `lms server start` if down. Use when the user wants a local-model answer via LM Studio. | `lmstudio`, `local model`, `lms`, `llm` |
...
| `omlx` | Query local oMLX server on :8080 (mlx_lm.server). Does NOT auto-start; user runs `omlx` in a foreground shell. Use when the user wants a local-model answer via oMLX. | `omlx`, `mlx`, `local model`, `llm` |
| `open` | Open files, folders, URLs, beads, and ~shortcuts in the default macOS app via the `open` command | `open`, `file`, `folder`, `url`, `shortcut` |
...
| `playwright-cli` | Automates browser interactions for web testing, form filling, screenshots, and data extraction. Use when the user needs to navigate websites, interact with web pages, fill forms, take screenshots, test web applications, or extract information from web pages. | `playwright`, `browser`, `screenshot`, `form`, `web test` |
...
| `silvery` | Debug and fix silvery rendering issues — incremental rendering, dirty flags, scroll containers, sticky children. Use when silvery renders incorrectly or has visual artifacts. | `silvery`, `rendering`, `dirty flags`, `scroll`, `visual artifacts` |
| `skill-improve` | Iterative skill refinement — run a skill, observe gaps, fix, re-run. Automated improvement loop. Use after /skill-test finds issues, or to polish any skill. | `skill improve`, `refine skill`, `polish skill` |
| `skill-test` | Pressure-test a skill with adversarial subagent scenarios. TDD for skills — write the test, watch it fail, fix the skill, watch it pass. | `skill test`, `adversarial`, `pressure test`, `tdd` |
...
| `sync` | Deprecated — split into /merge (reduce WIP) and /daily (cadence routines). This skill redirects. | `sync`, `merge`, `daily`, `deprecated` |
| `tdd` | TDD mode — reproduce first, fix second. Use PROACTIVELY when the user reports a bug, requests a feature, or says 'fix'. Also use when you catch yourself reading source code before writing a test. | `tdd`, `test first`, `reproduce`, `bug`, `fix` |
...
| `tribe` | Tribe coordination — check sessions, send messages, view health/history. Use when user says /tribe. | `tribe`, `sessions`, `agents`, `coordination` |
...
| `why` | 5 Whys root cause analysis + /big reframing. Use when the same area keeps breaking, when a fix feels like it's treating symptoms, or when you want to understand WHY a problem exists — not just how to fix it. | `why`, `root cause`, `five whys`, `reframe` |
