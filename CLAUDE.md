...
| agent dispatch / @agent/N / /claim / /do / km agent spawn / persona / slot / "assign work to an agent" / spawn a worker | docs/dev/agent-dispatch.md — canonical spec. 10 numeric slots @agent/0..9 at vault root; assignment = @agent/N or [[@agent/N]] in a bead item title (lands km:@agent/N in links table). The source of truth is `km bd query @agent/N`; a persisted slot board uses `# @agent/N km.add:: .` on the H1 and `## Queue km.default:: true` for initial placement. There are no km.add rules just because a file is named @agent/N.md. /claim @agent/N reserves a slot via repo.tryClaim (race-safe CAS, 20min agent / 24h user lease, in packages/km-beads/src/lease.ts). /do picks top-priority bead from any claimed slot's queue (composable with /loop). km agent spawn @agent/N is the out-of-process variant. |
