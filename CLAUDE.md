...
**Worktrees:** Pool slots live at `../<repo>-wtN` (sibling to repo) on plain branch `wtN`. Use `bun worktree create wtN` (not bare `git worktree`) — handles submodules, dependencies, hooks, and sets the right branch name for slot-pattern names. Run `bun worktree audit` to spot drift (legacy `.claude/worktrees/` locations, stuck rebases, dup commits, formatter-noise siblings). See [.claude/skills/worktree/SKILL.md] for the full rule.
...
The full rule, claim/release protocol, agent-spawn discipline, and incident log live in **[.claude/skills/worktree/SKILL.md](.claude/skills/worktree/SKILL.md)**. Load `/worktree` before spawning concurrent agents, asking "where do I work?", or planning anything that touches more than one branch. The 10 pool slots are sibling to the repo (`../<repo>-wt0`..`../<repo>-wt9`) on plain branches `wt0`..`wt9`, owned by the corresponding hats `@agent/0`..`@agent/9` — claiming a hat claims its worktree. Create or recycle a slot with `bun worktree create wtN`. `Agent({isolation: "worktree"})` is the rare fallback. **Never `git checkout <feature>` in the main repo.**
...
| sigil / @blah / #tag / +project / mention / board / "@-prefixed" / backlinks / km.add                                  | docs/architecture.md § "Links, Backlinks, and Materialization" + docs/concepts.md § Links + docs/design/model/klink.md + docs/design/model/storage.md § NodeRules. Rules: (1) @/#/+ are part of the node name, not separate namespaces. (2) Bare @blah ≡ [[@blah]] — both produce km:@blah in the canonical links table. (3) Backlinks are automatic and read-only. Persisted embed children require authored ![[...]] or a km.add rule on the owning outline item/H1. (4) km.add materializes item nodes by default, not body blocks; it does not distinguish bare sigils from wiki sigils. data.mentions / data.tags / data.projects are redundant denormalizations being deprecated; the links table is canonical.                                          |
...
| agent dispatch / @agent/N / /claim / /do / km agent spawn / hat / slot / "assign work to an agent" / spawn a worker    | docs/dev/agent-dispatch.md — canonical spec. 10 numeric hats @agent/0..9 at vault root; assignment = @agent/N or [[@agent/N]] in a bead item title (lands km:@agent/N in links table). Claiming @agent/N also claims worktree wtN. The source of truth is km bd query @agent/N; a persisted hat board uses # @agent/N km.add:: . km.default:: true on the H1 so generated embeds land top-level under the hat. There are no km.add rules just because a file is named @agent/N.md. /claim @agent/N reserves a hat via repo.tryClaim (race-safe CAS, 20min agent / 24h user lease, in packages/km-beads/src/lease.ts). /do picks top-priority bead from any claimed hat's queue (composable with /loop). km agent spawn @agent/N is the out-of-process variant. |
