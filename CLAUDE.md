...
Built-in slugs: `incremental` (tier 1, the historical incremental≡fresh check), `canary` (tier 1, degenerate-frame-after-first-render), `divergence` (tier 2, the blank-screen class-killer — frame painted real content cells but the in-process emulator readback is all-spaces, the `@km/code/v0.2/19604-focus-blank` signature; NO-OP for live TTYs since it only runs under `SILVERY_STRICT_TERMINAL=xterm|ghostty`), `cursor` / `cursor-position` (tier 2, managed-caret / hardware-cursor emission check — replays cursor-bearing output bytes through xterm.js and records semantic cursor, hardware parking/visibility, compositor caret, prompt/composer bounds, final cursor escape diagnostics, and a `cursor:row=N,col=M` breadcrumb; isolate with either slug), `bordered-rect-clip` (tier 2, cell painted outside the nearest bordered-Box ancestor's inner content rect — overflowing `<Text>` inside a `borderStyle` box without `overflow="hidden"`; skipped when the ancestor clips), `clip-parity` (tier 2, bg overlays from nested `<Text backgroundColor>` may only paint cells that text rendering painted under the same column clip — the bg-only-cells-past-the-border class; opt out with `SILVERY_STRICT=2,!clip-parity`), `bytes_out` (tier 1, render-out throughput monitor — WARN @ 1 MB/s × 10s, PANIC @ 100 MB/s × 2s, fixed thresholds, opt out with `SILVERY_STRICT=1,!bytes_out`; summaries include frame-cause attribution such as first render, dimension mismatch, changed cells, dirty rows, and output/cursor chars), `flicker` (tier 1, repaint-storm monitor — scans the SAME post-write terminal bytes `bytes_out` sees for destructive full-screen clears (`\x1b[2J`/`\x1b[3J`) and scroll-region resets (`\x1b[r`), and PANICs (throws, fail-loud) when ≥3 clears land in a 1s window with no intervening settle frame; the `@ag/code/20297-pane-flicker-on-resize` bare-`2J`-per-resize signature. Safe-by-construction against the 20297 _fix_: a legitimate homed full-cell repaint emits NO `2J` so it registers zero storm signals, and a single isolated clear never reaches the threshold. Fixed thresholds, opt out with `SILVERY_STRICT=1,!flicker`), `mem` (tier 1, in-process heap poll — 30s `process.memoryUsage()` samples, WARN when RSS or external+arrayBuffers doubles in 60s, opt out with `SILVERY_STRICT=1,!mem`), `layout-flag` (tier 1, layout-phase invariant — `layoutChangedThisFrame` is current-epoch only when `prevLayout` ≠ `boxRect`; slug-migrated from a raw env read in audit W1 #5), `layout-overflow` (tier 2, `strictLayoutOverflowCheck` — a non-absolute child's width exceeds its parent's inner content width when the parent doesn't clip; warn at tier 1, throw at tier 2; slug-migrated so compound specs like `incremental,2` now throw instead of silently warning), `scroll-invariants` (tier 2, `strictScrollInvariants` — scroll/sticky offset clamping + `scrollTo` / first- / last-visible viewport intersection + sticky `renderOffset` bounds; warn at tier 1, throw at tier 2; slug-migrated), `apportion-bands` (tier 2, `strictApportionBandsCheck` — per frame, over REALIZED widths: no `data-track-band` track below its band minimum while a sibling exceeds its band maximum; warn at tier 1, throw at tier 2. The subject is the laid-out geometry, NOT `apportion()`'s return value: the allocator's postcondition is provable, so checking it against itself is a tautology that can never fire. The defect lives downstream, where widths become flex props — a `grow` track carries no `maxWidth`, the fallback path floors tracks at the author's `minWidth` rather than the track's min-content, and a surface that still splits width with its own arithmetic breaks the contract without touching the allocator. The conjunction is load-bearing: starvation alone is legible degradation, surplus alone is a `grow` track absorbing free space; only together do they mean width was taken from a track that needed it and given to one that could not use it. Opt in by stamping `TRACK_BAND_ATTR` from `@silvery/ag`; `Table` does. Isolate with `SILVERY_STRICT=apportion-bands`, disable with `=2,!apportion-bands`), `fresh-layout` (tier 2, the `incremental` oracle's fresh baseline force-recomputes layout from scratch via `markLayoutTreeDirty` + `forceFullPropagate` instead of sharing the incremental path's just-cleaned flexily tree, so a missed-markDirty stale rect surfaces as incremental≠fresh — invisible to plain `incremental` whose fresh path skips `calculateLayout` via the ag.ts layout-on-demand gate; wired into all three STRICT sites, incl. the createApp/run production renderer that renders the @si/render/19436 follow-up standalone frame; deterministic per flexily's `expectRelayoutMatchesFresh` fuzz, so false-positive-free; isolate/disable with `SILVERY_STRICT=fresh-layout` / `=2,!fresh-layout`), and seven `<Island>` tier-2 slugs reserved by epic `@km/silvery/15646-islands`: `island-paint-oob` (guest writes outside its rect), `island-grapheme-width` (wide-cluster width disagreement), `island-resize-race` (writes at new dims before host confirms), `island-mode-leak` (protocol mode stuck after unmount/blur), `island-dispose-leak` (guest retains resources past dispose), `island-paint-budget` (per-island byte cadence), `island-boundary-limits` (mouse/focus coordinates leak past island borders). Observability instruments (`bytes_out`, `flicker`, `mem`) live at tier 1 — diagnostics that need ad-hoc env vars don't get used. See [`docs/guide/debugging.md`](docs/guide/debugging.md) for the full table and the tier-1 observability convention, and [`packages/ag-term/src/strict-mode.ts`](packages/ag-term/src/strict-mode.ts) for the `isStrictEnabled(slug, minTier)` helper.

## Mandatory: xterm.js in `@silvery/test` is REFERENCE-ONLY — do not "retire" it

`@silvery/test` depends on `@termless/xtermjs`, and that dependency is **not** a leftover from before vterm. It is load-bearing in the opposite direction from what a cleanup sweep assumes.

- `createTermless()` **is** the ANSI-fidelity harness, and xterm.js is the **differential reference** it measures against — including the `SILVERY_STRICT_TERMINAL` cross-check.
- vterm is the sole **production** shell guest (`@si/vterm/21016-terminal-runtime`). That ruling is about what ships, not about what proves what ships is correct.
- **Deleting this dependency does not retire xterm — it deletes the oracle.** Every backend divergence silently becomes undetectable, including the four where vterm is the faithful one and xterm degrades (real cursor shape, real cursor visibility, fancy underline styles, content-preserving narrowing reflow).
