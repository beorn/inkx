...
| agent dispatch / @agent/N / /claim / /do / km agent spawn / persona / slot / "assign work to an agent" / spawn a worker | docs/dev/agent-dispatch.md — canonical spec. 10 numeric slots @agent/0..9 at vault root; assignment = bare @agent/N in bead title (lands km:@agent/N in links table); slot self-aggregates via rules.add. /claim @agent/N reserves a slot via repo.tryClaim (race-safe CAS, 20min agent / 24h user lease, in packages/km-beads/src/lease.ts). /do picks top-priority bead from any claimed slot's queue (composable with /loop). km agent spawn @agent/N is the out-of-process variant (claim → compose brief → exec runtime; default --agent silvercode, alts claude/pi/headless-acp). Tribe-broadcasts at every transition. Tracking: @km/agent/sigil-boards. |
