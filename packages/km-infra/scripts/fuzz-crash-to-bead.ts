 ...
 */
// Strip ANSI escape sequences so regex matching is stable across reporters.
function stripAnsi(s: string): string {
  // biome-ignore lint/suspicious/noControlCharactersInRegex: ANSI escape pattern
  return s.replace(/\x1B\[[0-9;]*[A-Za-z]/g, "")
}

function parseCrashText(rawOutput: string): Crash | undefined {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Vitest failure-line formats we accept:
    //   FAIL  fuzz  packages/.../foo.fuzz.ts [ packages/.../foo.fuzz.ts ]
    //   FAIL  packages/.../foo.fuzz.ts > test name
    //    ❯ FAIL  packages/.../foo.fuzz.ts > test name
    //   ×  packages/.../foo.fuzz.ts
    // Strategy: if the line contains "FAIL" or starts with "×", scan for
    // the first token ending in .fuzz.ts(x) and extract that as the test
    // file path. Optional " > <test name>" suffix.
    if (!testFile && /(?:^|\s)(FAIL|×)\s/.test(line)) {
      const pathMatch = line.match(/(\S*\.fuzz\.tsx?)/)
      if (pathMatch) {
