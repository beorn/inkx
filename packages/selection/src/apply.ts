...
export function applyRemove<Sub>(
  ...
  if (state.cursor === id) {
    ...
    newCursor = state.cursor
    newAnchor =
      state.anchor !== null && remaining.indexOf(state.anchor) !== -1 ? state.anchor : newCursor
  }
...
export function applySelectAll<Sub>(
  ...
  const newCursor =
    state.cursor !== null && children.indexOf(state.cursor) !== -1 ? state.cursor : children[0]!
 ...
 */
export function applyTextEdit<Sub>(
  state: SelectionSnapshot<Sub>,
  nodeId: ID,
  offset: number,
): SelectionSnapshot<Sub> {
 ...
 * Set root ID.
 */
export function applySetRoot<Sub>(
  state: SelectionSnapshot<Sub>,
  id: ID | null,
): SelectionSnapshot<Sub> {
 ...
 */
export function assertInvariants<Sub extends SubSelectionBase>(
  state: SelectionSnapshot<Sub>,
): void {
  ...
  if (ids.length > 0 && cursor !== ids[0]) {
    throw new SelectionInvariantError(
      `cursor "${cursor}" !== ids[0] "${ids[0]}" (cursor must be first)`,
    )
  }
