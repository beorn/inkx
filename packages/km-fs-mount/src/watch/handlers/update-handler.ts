 ...
 */
// oxlint-disable-next-line complexity/complexity -- file-change reconciliation: .txt direct-write path + markdown re-parse path, each with stat/metadata/title/content/props/tags/anchors/child-edges update branches; flat sequential dispatch over the KNode fields a single file write might touch
export function handleUpdate(options: UpdateHandlerOptions): void {
  ...
  if (resolvedLinks.length > 0) {
    // Batch the inserts via a single prepared statement — bulk-sync calls
    // this once per file, and post-data.tags-dissolution each file may emit
    // many hashtag link rows; per-row `addLink` re-prepared on every call.
    addLinks(
      db,
      resolvedLinks.map((link) => ({
        host_id: idMap.get(link.host_id) ?? link.host_id,
        href: link.href,
        rel: link.rel,
      })),
    )
