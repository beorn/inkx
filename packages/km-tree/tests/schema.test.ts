...
describe("canHaveChildren", () => {
  test("item node can have children", () => {
    expect(canHaveChildren({ item: true })).toBe(true)
  })

  test("block node (no item) cannot have children", () => {
    expect(canHaveChildren({})).toBe(false)
    expect(canHaveChildren({ item: false })).toBe(false)
...
describe("canParent", () => {
  test("item parent can accept any child", () => {
    expect(canParent({ item: true }, { item: true })).toBe(true)
    expect(canParent({ item: true }, {})).toBe(true)
  })

  test("block parent cannot accept children", () => {
    expect(canParent({}, { item: true })).toBe(false)
...
describe("canBecomeBlock", () => {
  test("childless item can become block", () => {
    ...
    const parentId = repo.addNode(null, { type: "h", item: true, name: "Section", content: "Section" })
    const childId = repo.addNode(parentId, { type: "p", item: true, content: "Leaf item", parent_idx: 1 })
  ...
  test("item with children cannot become block", () => {
    const repo = createTestRepo()
    const parentId = repo.addNode(null, { type: "h", item: true, name: "Section", content: "Section" })
    const childId = repo.addNode(parentId, { type: "p", item: true, content: "Has kids", parent_idx: 1 })
