...
describe("INDENT action", () => {
  test("indents second child under first child", () => {
    ...
    const action: OutlinerOp = { type: "INDENT", nodeId: bId }

    const [newState, effects] = applyOutlinerOp(repo, state, action)
  ...
  test("first child indent is no-op with bell", () => {
    ...
    const action: OutlinerOp = { type: "INDENT", nodeId: aId }
  ...
  test("nonexistent node indent emits bell", () => {
    ...
    const action: OutlinerOp = { type: "INDENT", nodeId: "nonexistent" }

    const [_, effects] = applyOutlinerOp(repo, state, action)
    expect(hasEffect(effects, "bell")).toBe(true)
  })
})
...
describe("OUTDENT action", () => {
  test("nested child outdents to grandparent level", () => {
    ...
    const action: OutlinerOp = { type: "OUTDENT", nodeId: child1Id }
  ...
  test("root-level node cannot outdent — bell", () => {
    ...
    const action: OutlinerOp = { type: "OUTDENT", nodeId: sectionAId }
...
describe("INDENT then OUTDENT = identity", () => {
  test("indent then outdent restores original parent", () => {
    ...
    // Indent B under A
    const [stateAfterIndent, _] = applyOutlinerOp(repo, state, { type: "INDENT", nodeId: bId })
    expect(stateAfterIndent.nodes.get(bId)!.parent_id).not.toBe(parentId)

    // Outdent B back
    const [stateAfterOutdent, __] = applyOutlinerOp(repo, stateAfterIndent, {
      type: "OUTDENT",
      nodeId: bId,
    })
...
describe("MOVE_UP action", () => {
  test("moves node before its previous sibling", () => {
    ...
    const [newState, effects] = applyOutlinerOp(repo, state, { type: "MOVE_UP", nodeId: bId })
  ...
  test("first child cannot move up — bell", () => {
    ...
    const [_, effects] = applyOutlinerOp(repo, state, { type: "MOVE_UP", nodeId: aId })
    expect(hasEffect(effects, "bell")).toBe(true)
    expect(hasEffect(effects, "persist")).toBe(false)
  })
})
...
describe("MOVE_DOWN action", () => {
  test("moves node after its next sibling", () => {
    ...
    const [newState, effects] = applyOutlinerOp(repo, state, { type: "MOVE_DOWN", nodeId: bId })
  ...
  test("last child cannot move down — bell", () => {
    ...
    const [_, effects] = applyOutlinerOp(repo, state, { type: "MOVE_DOWN", nodeId: cId })
    expect(hasEffect(effects, "bell")).toBe(true)
    expect(hasEffect(effects, "persist")).toBe(false)
  })
})
...
describe("MOVE_UP then MOVE_DOWN = identity", () => {
  test("move up then move down restores original order", () => {
    ...
    const [midState, _] = applyOutlinerOp(repo, state, { type: "MOVE_UP", nodeId: bId })
    const [finalState, __] = applyOutlinerOp(repo, midState, { type: "MOVE_DOWN", nodeId: bId })
...
describe("SPLIT_BLOCK action", () => {
  test("splits at cursor middle", () => {
    const { repo, bId, state } = setupFlatList()
    const action: OutlinerOp = { type: "SPLIT_BLOCK", nodeId: bId, cursorOffset: 3 }
  ...
  test("split at end creates empty sibling after", () => {
    ...
    const action: OutlinerOp = { type: "SPLIT_BLOCK", nodeId: bId, cursorOffset: 5 }
  ...
  test("focus effect points to new node", () => {
    const { repo, bId, state } = setupFlatList()
    const [_, effects] = applyOutlinerOp(repo, state, {
      type: "SPLIT_BLOCK",
      nodeId: bId,
      cursorOffset: 3,
    })
...
describe("MERGE_BLOCK backward", () => {
  test("merges plain p with previous sibling", () => {
    ...
    const [newState, effects] = applyOutlinerOp(repo, freshState, {
      type: "MERGE_BLOCK",
      nodeId: bId,
      direction: "backward",
    })
  ...
  test("degradation (strip task) does not delete the node", () => {
    ...
    const [newState, effects] = applyOutlinerOp(repo, state, {
      type: "MERGE_BLOCK",
      nodeId: taskId,
      direction: "backward",
    })
...
describe("DELETE_NODE action", () => {
  test("deletes a node and focuses next sibling", () => {
    const { repo, bId, cId, state } = setupFlatList()
    const action: OutlinerOp = { type: "DELETE_NODE", nodeId: bId }
  ...
  test("deletes last child and focuses previous sibling", () => {
    const { repo, bId, cId, state } = setupFlatList()
    const action: OutlinerOp = { type: "DELETE_NODE", nodeId: cId }
...
describe("effect correctness", () => {
  test("successful actions always emit persist", () => {
    const { repo, bId, state } = setupFlatList()
    const actions: OutlinerOp[] = [
      { type: "INDENT", nodeId: bId },
      { type: "SPLIT_BLOCK", nodeId: bId, cursorOffset: 2 },
    ]

    // Test indent
    const [_, indentEffects] = applyOutlinerOp(repo, state, actions[0]!)
  ...
  test("failed actions never emit persist", () => {
    ...
    const [_, effects] = applyOutlinerOp(repo, state, { type: "INDENT", nodeId: aId })
    expect(hasEffect(effects, "persist")).toBe(false)
  })

  test("node_moved effect carries correct from/to parent", () => {
    const { repo, parentId, aId, bId, state } = setupFlatList()
    const [_, effects] = applyOutlinerOp(repo, state, { type: "INDENT", nodeId: bId })
