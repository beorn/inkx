 ...
 * ```
 *
 * ## Reuse — this composes existing silvery machinery (DRY audit)
 *
 * Every rendering piece delegates to a primitive that already existed; nothing
 * is re-implemented:
 * - inline emphasis / code → the `Strong` / `Em` / `Code` typography presets
 * - headings → `H1`–`H6`; horizontal rule → `HR`; blockquote → `Blockquote`
 * - bullet / ordered lists → `UL` / `OL` / `LI` (markers, hanging indent,
 *   depth-aware nesting, and per-item reflow all come from the presets)
 * - fenced code → `CodeBlock`, one per source line (its `│` gutter + `wrap="hard"`)
 * - the outer wrapping column → `Prose`
 * - paragraph body → `P`; every span wraps via the `Text` primitive
 *
 * What did NOT already exist — the reason this component is net-new — is the
 * Markdown *parser*: block classification and, above all, **paragraph reflow**
 * (joining an author's hard-wrapped lines so the terminal re-wraps to the pane
 * width). Silvery shipped every rendering primitive but no Markdown reader;
 * `@silvery/syntax` is code syntax highlighting (async Shiki), not Markdown; and
 * km's Markdown path (`km-tui` / `@km/md`) can't be imported here (vendor
 * independence). So this file is *parser + composition*, not new rendering.
 *
...
/**
 * Render a list via the `UL` / `OL` / `LI` presets. `LI` supplies the bullet
 * glyph (depth-aware through the shared `ListContext`), the hanging indent, and
 * item-text reflow; `OL` auto-numbers its `LI` children.
 *
 * A sub-list is emitted as an indented SIBLING of its parent `LI` (still inside
 * the same `UL`/`OL`), not as a child of the `LI`. `LI` wraps its children in a
 * `<Text>`, and silvery forbids a `<Box>` (which every list is) inside a
 * `<Text>`; placing the sub-list as a sibling keeps it a block while still
 * inheriting the parent list's context, so it renders one depth deeper. `OL`
 * only numbers direct `LI` children, so a sibling sub-list never perturbs the
 * numbering.
 */
function renderList(list: MdList, key: string): JSX.Element {
  const List = list.ordered ? OL : UL
  return (
    <List key={key}>
      {list.items.flatMap((item, index) => {
        const itemKey = `${key}-${index}`
        const li = <LI key={itemKey}>{parseInline(item.text, itemKey)}</LI>
        return item.list === undefined ? [li] : [li, renderList(item.list, `${itemKey}-sub`)]
      })}
    </List>
  )
}
...
function renderBlock(block: MdBlock, key: string): ReactNode {
  switch (block.kind) {
    ...
    case "paragraph":
      return <P wrap="wrap">{parseInline(block.text, key)}</P>
    case "code":
      // One CodeBlock per source line preserves the authored breaks (each line
      // char-wraps, never reflows) and keeps the preset's `│` gutter per line.
      return (
        <Box flexDirection="column" minWidth={0}>
          {block.lines.map((line, index) => (
            <CodeBlock key={index}>{line === "" ? " " : line}</CodeBlock>
          ))}
        </Box>
      )
    case "quote":
 ...
 * supported subset and the paragraph-reflow contract. The outer column is a
 * `Prose` (the canonical long-form-text container — it supplies the
 * `flexDirection: column` + `flexShrink` + `minWidth: 0` wrap chain); any
 * `BoxProps` (padding, width, color, …) pass through, and `gap` defaults to 1
