...
2. [`src/kast/`](src/kmast) — the km-ast types and what gets mapped to/from mdast; [`src/ast2nodes.ts`](src/ast2nodes.ts) and [`src/nodes2md.ts`](src/nodes2md.ts) are the two conversion directions
