...
// PROP_REGEX, ExtractedProp, extractKVProperties moved to @km/core (heading-rules.ts)
// Re-exported below for backward compatibility within this module.
...
// SectionRules, ParsedHeading, parseHeadingRules moved to @km/core (heading-rules.ts)
// SectionRules is now NodeRules (from @km/core/types.ts) — they were identical types.

/**
 * Serialize NodeRules back to km.key:: value format for heading reconstruction.
 * Returns the space-separated string (empty string if no rules).
 */
export function serializeRules(rules: NodeRules): string {
  const parts: string[] = []
  if (rules.add) {
