...
const setPriority = {
  id: "set_priority",
  name: "Cycle Priority",
  description: "Cycle task priority (none → P0 → P1 → … → P4 → none)",
  category: "Task",
  shortLabel: "priority",
  execute: (ctx) => {
    if (!ctx.currentNodeId) return null
...
function createSetPriorityCommand(level: number): CommandDef {
  return {
    ...
    execute: (ctx) => {
      if (!ctx.currentNodeId) return null
      return { type: "SET_PRIORITY", nodeId: ctx.currentNodeId, level }
    },
  }
}
