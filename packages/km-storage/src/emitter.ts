...
export interface EmitOptions {
  /** Skip persisting the event row to the events table */
  skipPersist?: boolean
...
export interface Emitter {
  ...
  /**
   * Apply a change to the system (DB + events table + broadcast + onApply callbacks).
   *
   * 1. Applies to database (if db provided) — nodes row + events row commit
   *    atomically inside one SAVEPOINT
   * 2. Broadcasts via changeHub (unless skipBroadcast)
   * 3. Notifies onApply subscribers (FS projection, etc.)
   */
...
export function createEmitter(options: EmitterOptions): Emitter {
  ...
  /**
   * Internal: apply to DB + events table + broadcast.
  ...
  function commitInternal(partialChange: Omit<Change, "id" | "ts">, commitOptions: EmitOptions = {}): Change {
    ...
    // Apply to database — nodes row + events row commit atomically inside
    // one SAVEPOINT. SAVEPOINT works whether the caller is already in a
    ...
    // Broadcast via change hub — isolated
...
/**
 * Insert one row into the `events` table — the canonical event log since
 * SCHEMA_VERSION 12 (see @km/storage/events-table-replaces-jsonl). We split
 * actor/source/target out of the Change shape so the SQL columns can be
 * indexed directly.
 *
 ...
 *
 * UNIQUE(id) conflicts are expected during replay (same event id arrives
 * twice) and are surfaced via SQLite's constraint-violation error; callers
 * may choose to treat them as benign.
 */
