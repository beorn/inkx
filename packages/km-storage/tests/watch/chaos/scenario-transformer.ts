 ...
 *
 * Transforms events based on chaos scenario configuration.
 */

import { dirname, basename, join } from "path";
import type { ScheduledEvent, ChaosScenario, FsEvent } from "./types.ts";
...
/**
 * Slow disk: Add random delay (2-5s by default) to all events
 */
...
/**
 * Queue overflow: Drop events randomly based on dropRate
 */
...
/**
 * Editor atomic: Convert change events to unlink + add pairs
 * Simulates editors that write to temp file then rename
 */
...
/**
 * Event storm: Emit events in rapid succession with minimal delays
 */
...
/**
 * Reorder chaos: Shuffle events within windows
 */
function applyReorderChaos(
  ...
  // Only reorder if probability hits
...
/**
 * Partial writes: Emit add/change followed by multiple intermediate change events
 */
...
/**
 * Rename storm: Convert add events into chains of renames
 */
...
/**
 * FSEvents coalesce: Merge many file events into a single directory event
 */
function applyFsEventsCoalesce(
  ...
  for (const [dir, dirEvents] of byDir) {
    if (dirEvents.length > threshold && useParentDir) {
      ...
      // This simulates FSEvents kFSEventStreamEventFlagMustScanSubDirs
      result.push({
        type: "change",
        path: dir,
        timing: { delay: 0 },
...
/**
 * Init gap: Delay all events to simulate watcher initialization period
 */
function applyInitGap(
  ...
  // All events happen before watcher is "ready"
  // These events might be lost depending on implementation
  return events.map((e, i) => ({
    ...e,
    timing: {
      ...e.timing,
      // Events spread throughout init period
...
/**
 * Rapid succession: Duplicate events many times with tiny delays
 */
function applyRapidSuccession(
  ...
  for (const event of events) {
    ...
    // Emit multiple change events in rapid succession
