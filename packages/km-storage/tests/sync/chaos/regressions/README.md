...
Regression cases are now managed automatically by vimonkey.

## How It Works

When a `test.fuzz` test fails, vimonkey:
...
infrastructure has been replaced by vimonkey's `test.fuzz` + `gen()`/`take()`.
