...
describe.sequential("Parallel Suite Runner", () => {
  ...
  test("runs single repo with multiple scenarios", async () => {
    const result = await runChaosSuiteParallel({
      repoCount: 1,
  ...
  test("groups results correctly by repo and scenario", async () => {
    const result = await runChaosSuiteParallel({
      repoCount: 2,
      scenarios: [NO_CHAOS, CHAOS_SCENARIOS.reorder_chaos],
      parallel: false,
    ...
    // 2 repos × 2 scenarios = 4 total tests
    expect(result.summary.total).toBe(4)

    // Each repo should have 2 results (one per scenario)
    expect(result.byRepo.get(0)?.length ?? 0).toBe(2)
    expect(result.byRepo.get(1)?.length ?? 0).toBe(2)

    // Each scenario should have 2 results (one per repo)
