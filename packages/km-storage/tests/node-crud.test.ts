...
import { withTestEnv, withTestEnvSync } from "./test-utils.ts";
...
describe("Node CRUD Operations", () => {
  ...
  describe("Bidirectional Sync", () => {
    test("should write task status change back to markdown file", () =>
      withTestEnv(async ({ kmDir }) => {
        setDatabase({ applyEvent });

        // Create a test markdown file with a task
        const testFile = join(kmDir, "..", "test-task.md");
    ...
    test("should write blocked status to markdown file", () =>
      withTestEnv(async ({ kmDir }) => {
        setDatabase({ applyEvent });

        const testFile = join(kmDir, "..", "test-blocked.md");
    ...
    test("should write dropped status to markdown file", () =>
      withTestEnv(async ({ kmDir }) => {
        setDatabase({ applyEvent });

        const testFile = join(kmDir, "..", "test-dropped.md");
