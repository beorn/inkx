...
State persisted to `.agents/skills/sop/state.json` (last run times + findings per domain).
...
- [ ] `doc-sync` — are README, homepage (docs/index.md), AGENTS.md, and GitHub description consistent? Tagline, feature list, code examples, and competitive claims should match across all four surfaces. See sync set below.
...
| **AGENTS.md** | AGENTS.md | Tagline, quick start example |
...
**Assets**: CI workflows, git hooks, .agents/skills/*, MCP configs, tool versions, accountly, Cloudflare zones/domains/workers
**Cadence**: monthly
...
sites.doc-change(README) → sites.check(doc-sync)  # keep homepage, AGENTS.md, GH description in sync
```
...
State stored in `.agents/skills/sop/state.json`:
```json
{
  "lastRun": {
