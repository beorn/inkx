...
Tool: `.agents/skills/npm/registry.ts`. Run via `bun npm-registry <cmd>`. All registry calls are cached for 5 minutes in `/tmp/.npm-registry-cache.json` so subsequent commands are cheap. Delete the cache to force-refresh.

Canonical registry file: [`.agents/skills/release/npm-packages.md`](../release/npm-packages.md). The audit subcommand cross-checks this file against the live registry.
