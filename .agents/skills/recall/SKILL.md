---
description: Search and manage Codex session history. Use proactively when encountering errors, starting work, or recovering lost content.
...
Unified CLI for searching, managing, and recovering Codex session history.
...
`tribe@bearly` plugin or a project `.mcp.json` entry) — they call the same
library as `bun recall` but without the ~400ms subprocess-spawn cost. The CLI
remains available for humans and scripts.
...
1. Writes `~/.codex/bearly-sessions/pid-<Codex-pid>.json` (sentinel fallback)
...
Background index logs go to `~/.codex/bearly-sessions/index-bg.log`. Set
...
# Offline trace for post-hoc A/B evaluation — writes JSON to ~/.codex/recall-traces/
...
- Database: `~/.codex/session-index.db`
