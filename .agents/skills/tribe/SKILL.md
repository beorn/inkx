...
| `tribe.ask` / `tribe.brief` / `tribe.plan` / `tribe.session` / `tribe.workspace` / `tribe.inject_delta` | Lore (session history / memory) tools — see `.agents/skills/recall/` |
...
- **Stdio adapter** (`stdio-adapter.ts`): Per-agent transport translator — bridges Codex's stdio MCP to the daemon's Unix-socket MCP. Forwards tool calls; no DB access.
