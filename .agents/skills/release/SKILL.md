...
The intelligence lives in Codex reading diffs and writing changelogs. The mechanics live in thin, testable tools.
...
| **diffs** | Dump unreleased commits + full diffs for AI to read | `bun .agents/skills/release/diffs.ts [repo]` |
| **propose** | AI reads diffs, drafts bump decisions + changelog entries | Codex (prompted by skill) |
...
1. Run `bun .agents/skills/release/diffs.ts <repo>` to get structured commits + full diffs
2. **Codex reads the diffs and proposes:**
...
**Dependent bump policy**: If package A has a major bump and package B re-exports A's types/API, B also needs a major. Codex checks each consumer explicitly.
...
- `diffs.ts` — dumps unreleased commits + diffs for Codex to read
