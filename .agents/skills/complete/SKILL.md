...
2. **Check the shadow copies.** Code gets described in multiple places: source, tests, test fixtures, docs, skill files, AGENTS.md, MEMORY.md, `docs/ref/visual-spec.md`, error messages, log strings, comments. A change to the source that doesn't update the shadows leaves lies behind.
...
Close completed beads. Commit and push (beads ride normal git transport — no separate sync step).
...
- *(`/round-close` was folded into this skill on 2026-04-29 — its Iron Rule and acceptance-grep replay live in the preamble above.)*
