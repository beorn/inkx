...
interface MouseEventProps {
  ...
  onMouseEnter?: (event: SilveryMouseEvent) => void // No bubble (same as DOM)
  onMouseLeave?: (event: SilveryMouseEvent) => void // No bubble (same as DOM)
...
interface SilveryMouseEvent {
  ...
  target: SilveryNode // Deepest node under cursor
  currentTarget: SilveryNode // Node whose handler is firing (changes during bubble)
...
The runtime already has access to the root `SilveryNode`. After `screenRectPhase` runs, every node has its screen position. We walk this tree for hit testing.
...
interface KeyboardEventProps {
  onKeyDown?: (event: SilveryKeyboardEvent) => void
  onKeyUp?: (event: SilveryKeyboardEvent) => void
}

interface SilveryKeyboardEvent {
...
```tsx
// In silvery — mirrors HTML's <a> tag
