...
```
      ...
      (JSONL eval)   (NL agent)      (human REPL)
      withCode()     withAIAgent()   withRepl()
          │               │               │
        JSONL           tRPC            Text
          │               │               │
          └───────────────┼───────────────┘
...
- **CRDT-backed state (TODO — worth exploring).** What if all app state lives in a CRDT (like Automerge) and every command produces a change on it? This would give us:
  ...
  - This would make the command/effect split from [TEA (The Elm Architecture) state machines](../../docs/design/tea-state-machines.md) even more meaningful: commands that produce state changes are always safe (CRDT-backed, undoable). Commands that produce effects (send email, write file) are the only ones that need confirmation.
