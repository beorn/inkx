...
Selection operates on the **render tree**, not the terminal buffer. The same way `window.getSelection()` in a browser walks DOM nodes, silvery's selection walks `TeaNode`s.
...
interface NodePosition {
  /** The TeaNode containing the text (type: "silvery-text", isRawText: true) */
  nodeId: string
 ...
 *
 * 1. hitTest(root, x, y) → TeaNode (deepest node at position)
 ...
 */
function hitTestText(root: TeaNode, x: number, y: number): NodePosition | null
```
 ...
 */
function resolveSelectionText(root: TeaNode, range: SelectionRange): string
```
...
The selection overlay is applied in the **content phase**, after text rendering and before output. For each selected cell:

```typescript
// In the content phase, after rendering text to the buffer:
...
OSC 52 is already implemented in `@silvery/term/clipboard.ts`. It works across SSH, in tmux, and in all modern terminals (Ghostty, Kitty, WezTerm, iTerm2, xterm, foot).
...
Post-process buffer to invert selected cells. Wire into content phase with `selectionDirty` flag.
