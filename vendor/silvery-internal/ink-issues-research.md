...
The issue that started it all — open since 2016. Components can't know their size during render. Documented in the opening paragraph of `silvery-vs-ink.md` and in `why-silvery.md`. Silvery's `useContentRect()` is the direct answer.
...
3. Ink-UI's issues are almost entirely solved by `@silvery/ui` — users get a better component library included
...
After migration, you can incrementally adopt Silvery-only features: `useContentRect()` to remove width prop drilling, `overflow="scroll"` to remove virtualization code, `InputLayerProvider` to remove manual input guards, `@silvery/theme` for consistent styling. Each improvement is independent — adopt them at your own pace.
