...
import { type Layout, type MeasureFunc, type Style, type Value, createDefaultStyle } from './types.js';
...
function setEdgeValue(arr: [Value, Value, Value, Value], edge: number, value: number, unit: number): void {
	const v = { value, unit };
	switch (edge) {
		case C.EDGE_LEFT:
			arr[0] = v;
...
function setEdgeBorder(arr: [number, number, number, number], edge: number, value: number): void {
	switch (edge) {
		case C.EDGE_LEFT:
			arr[0] = value;
			break;
 ...
 */
function computeLayout(root: Node, availableWidth: number, availableHeight: number): void {
...
function layoutNode(
	...
	if (node.hasMeasureFunc() && node.children.length === 0) {
		...
		const widthMode = style.width.unit === C.UNIT_AUTO ? C.MEASURE_MODE_AT_MOST : C.MEASURE_MODE_EXACTLY;
		const heightMode = style.height.unit === C.UNIT_AUTO ? C.MEASURE_MODE_UNDEFINED : C.MEASURE_MODE_EXACTLY;
	...
	if (relativeChildren.length > 0) {
		const isRow = style.flexDirection === C.FLEX_DIRECTION_ROW || style.flexDirection === C.FLEX_DIRECTION_ROW_REVERSE;
		const isReverse = style.flexDirection === C.FLEX_DIRECTION_ROW_REVERSE || style.flexDirection === C.FLEX_DIRECTION_COLUMN_REVERSE;
		...
		for (const child of relativeChildren) {
			...
			const mainMargin = isRow
				? resolveValue(cs.margin[0], mainAxisSize) + resolveValue(cs.margin[2], mainAxisSize)
				: resolveValue(cs.margin[1], mainAxisSize) + resolveValue(cs.margin[3], mainAxisSize);
		...
		for (let i = 0; i < ordered.length; i++) {
			...
			if (child.hasMeasureFunc() && child.children.length === 0) {
				...
				if (widthAuto || heightAuto) {
					...
					const availW = widthAuto ? (isRow ? childMainSize : crossAxisSize - crossMargin) : cs.width.value;
					const availH = heightAuto ? (isRow ? crossAxisSize - crossMargin : childMainSize) : cs.height.value;
 ...
 */
function applyMinMax(size: number, min: Value, max: Value, available: number): number {
	let result = size;
