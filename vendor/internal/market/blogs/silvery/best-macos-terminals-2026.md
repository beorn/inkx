...
# Comparing macOS Terminal Emulators in 2026

I maintain [terminfo.dev](https://terminfo.dev), a database that probes terminals for escape sequence support. Instead of relying on documentation claims, it runs actual tests against each terminal and records what passes. This article uses that data to compare six macOS terminals I've used seriously: Ghostty, Kitty, iTerm2, WezTerm, Warp, and Terminal.app.

I'm not going to rank these #1 through #6. They make different tradeoffs, and which one fits best depends on what you care about.
...
**Kitty keyboard protocol.** The most useful keyboard protocol change I've used. Traditional terminals can't distinguish Ctrl+I from Tab, or Ctrl+M from Enter. The Kitty protocol fixes this with unambiguous key reporting, modifier detection, and key release events.

Supported by: Ghostty, Kitty, iTerm2, WezTerm, Warp. Not supported by Terminal.app.

**Truecolor (24-bit RGB).** Full-spectrum color instead of the 256-color palette.

Supported by all six terminals.
...
All six support clipboard writing. OSC 52 reading is more restricted -- only iTerm2 enables it by default, and for good reason: allowing any application to silently read your clipboard is a security concern.
...
- **Kitty graphics protocol**: Supported by Ghostty, Kitty, Warp, and Terminal.app. Not supported by iTerm2 or WezTerm.
- **Sixel**: Supported by Terminal.app and WezTerm. Not supported by Ghostty, Kitty, or Warp.
- **iTerm2 image protocol (OSC 1337)**: Supported by iTerm2 and some other terminals that added compatibility.

The Kitty graphics protocol is more capable (animation, transparency, arbitrary placement) but sixel has broader cross-platform adoption. WezTerm and Terminal.app are the macOS terminals with sixel support.
...
Ghostty, Kitty, WezTerm, and Warp all use GPU-accelerated rendering. Alacritty pioneered this approach in 2017, and it's become the standard for new terminals. The practical effect in my experience: scrolling tends to feel smoother, and high-DPI displays are handled well.
...
I use Ghostty as my primary terminal. It starts fast (~50ms to first frame in my testing), handles large output without lag, and its standards compliance means TUI applications have worked reliably for me. The lack of built-in tabs bothered me initially, but I use a tiling window manager (AeroSpace) so I rarely need them.
...
iTerm2 still feels like the most feature-dense terminal on macOS to me. Split panes, tmux integration, Instant Replay, triggers, password manager, shell integration -- it has features I haven't found in other terminals. If you want everything in one application and don't care about GPU rendering, iTerm2 is hard to beat.

WezTerm occupies an interesting niche. The Lua configuration is genuinely powerful -- I've seen people build complex multi-session setups that would be difficult with static config files. The built-in multiplexer with SSH domain support means you can drop tmux entirely.

Warp takes a different approach entirely. Its blocks-based UI groups each command with its output, AI-assisted command suggestions are built in, and the input area is a real text editor with selection and multi-cursor. It scored 89.6% on terminfo.dev, which puts it solidly in the modern tier for protocol support. If you're interested in where terminal UIs might be heading, Warp is worth trying.
...
If you want programmable configuration: **WezTerm**. I haven't found another terminal that lets you express complex conditional behavior in your terminal config the way Lua does.
