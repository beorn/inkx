...
// simplified — assumes a truncate(text, maxLen) helper exists
...
This works, but the component renders twice -- first with `width=0`, then with the real width. With nested responsive components (board -> column -> card), each level goes through its own measure-rerender cycle. In a small prototype, I saw multiple settling passes before the layout converged.
...
function Card({ item }: { item: Item }) {
  ...
  return (
    <Box>
      <Text>{truncate(item.title, Math.max(0, width - 4))}</Text>
...
`overflow="scroll"` works because the framework knows how tall the container is and how tall the children are. Silvery measures all children, determines which are visible in the viewport, and renders only those. For most cases, variable-height children are handled without needing an `estimateHeight` function.
...
For incremental updates (pressing a key, scrolling, typing), in my benchmarks, Silvery is noticeably faster for localized updates. Most of the pipeline is skipped because the caching knows exactly what changed. A typical interactive update -- moving a cursor in a large tree -- completes in sub-millisecond time (see the [benchmark comparison page](https://silvery.dev/benchmarks) for methodology and numbers).
