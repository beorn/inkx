...
- **Claude Code "focus mode" is a render-layer projection, not a transcript-layer fact.** Claude Code's official `focus` toggle: *"show only your prompt, a tool summary, and the final response"* — earlier assistant text emissions and per-tool detail are collapsed under "N messages hidden (focus to show)". The underlying transcript still contains every narration op and every tool call interleaved; focus is a per-turn projection rendered as `[user prompt] → [compact tool summary line, e.g. "Ran 3 shell commands"] → [final assistant text]`. Implication for parity: transcript parity (which message ops landed) is unaffected, but **visual replay parity** (`l5-visual-replay-parity`) must decide explicitly — replicate the focus-collapse with the same three-part shape, render full transcripts always, or expose it as a per-view toggle. Treat focus mode as a partial/optional capability fact in the phase 08 matrix. A turn legitimately has 1..N narration emissions; assuming "one narration per turn" because Claude Code's focus mode showed one is a category error.
