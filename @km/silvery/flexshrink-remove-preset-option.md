...
6. Update doc comment: silvery uses CSS-correct defaults; consumers wanting Yoga-compat use flexily directly
7. Update the four call sites to drop the 'css' arg:
...
14. tests/compat/ink/helpers/render-to-string.ts: needs separate Yoga-flavored path (createFlexily directly with yoga preset, OR re-add yoga arg via different mechanism)
15. apps/silvercode/tests/visual/paragraph-wrap.test.tsx + message-wrap-truncation.test.tsx: these still call createFlexilyZeroEngine() bare — verify they still work after the cleanup
