...
6. **flexily**: switch auto-min-size to spec-correct min-content (revert max-content approximation in layout-zero.ts)
...
11. **silvery TextProps**: extend FlexboxProps so Text accepts flexShrink/flexGrow/flexBasis/minWidth
...
15. **silvery whiteSpace / textWrap inheritance**: one prop at row level instead of per-child
