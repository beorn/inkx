...
- [ ] km-beads: reconcile data-schema integration with deps-table approach #task #P2 @issue priority:: P2
  ...
  2. KEEP the deps-table reads in queries.ts (don't revert to JSON scan)
  3. Update the integration test (data-schema-integration.test.ts content from b982f72ce) to assert against the deps-table-shaped read path
...
- To finish
2. Re-implement the data-schema wiring in mutations.ts + deps.ts (take wt4's Zod parse-on-write approach for data integrity)
3. KEEP the deps-table reads in queries.ts (don't revert to JSON scan)
4. Update the integration test (data-schema-integration.test.ts content from b982f72ce) to assert against the deps-table-shaped read path
