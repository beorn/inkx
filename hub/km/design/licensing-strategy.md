...
**Voice correction (2026-04-28)**: prior drafts proposed leading with "Drop-in for Ink. 98.6% compatible." or similar wedge headlines. Founder's call: too direct, risks Ink-community backlash (Vadim is well-liked; aggressive migration framing reads as patricide), and the bold claims aren't fully battle-tested at 3K DL/wk — they may not hold under broader user load. **The migration capability stays; the marketing voice softens.** Bigger / sharper claims become available *after* third-party validation lands (case study + outside maintainers re-running benchmarks). Sequence: ship → measure → validate → escalate claims. Don't pre-burn credibility.
...
1. **silvery.dev hero rewrite (softened, 2026-04-28)** — keep silvery's existing identity ("React for modern terminal apps. Powerful apps. Polished UIs.") and **add** Ink compatibility as a feature, not the headline. Working candidate: subhead mentions "Compatible with Ink — keep your code if you need," and "Migrating from Ink" appears as a third-tier nav item, not the primary CTA. Don't lead with "Drop-in for Ink" or "98.6% compatible" — too direct, risks Ink-community backlash, and the bold quantitative claims should wait for third-party validation. Bigger claims become available *after* the case study + outside benchmark validation. Move performance multipliers off hero into a Performance page with reproducible scripts.
