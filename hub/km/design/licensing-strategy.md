...
**Decision (locked, post-/pro 4-leg final review, 2026-04-27 night)**: open silvery at **Apache-2.0 + CLA**. The architectural *position* is that silvery is uniquely capable of being the React-Ink-migration target (architecture built specifically for that purpose). The marketing *voice* keeps silvery's existing identity ("the next-generation React TUI framework") with Ink compatibility surfaced as a capability, not the headline. silvercode + km stay proprietary. tribe = open wire/spec (Apache + CC BY 4.0) + proprietary hosted gateway. The **SDK-with-services** revenue pattern (Stripe Elements / Mapbox SDK) — open silvery components funnel to paid backend services (`<AgentSession>`, `<RecallPanel>`, `<TribeRoom>`, `<SquadView>`, `<ContextSafety>`, etc.).

**Voice correction (2026-04-28)**: prior drafts proposed leading with "Drop-in for Ink. 98.6% compatible." or similar wedge headlines. Founder's call: too direct, risks Ink-community backlash (Vadim is well-liked; aggressive migration framing reads as patricide). The 98.6% Ink-compat and 3-27× faster claims themselves **are verified and stay on the page** — those numbers are battle-tested and visible on silvery.dev today. The correction is voice, not claims: **don't lead** the hero with the migration framing; keep the perf/compat numbers as features in their existing presentation. Bigger / sharper *positioning* claims (e.g. "the Ink-migration framework") become available *after* third-party validation lands (case study + outside maintainers re-running benchmarks). Sequence: ship → measure → validate → escalate positioning. Don't pre-burn credibility on framing.
...
1. **silvery.dev hero rewrite (softened, 2026-04-28)** — keep silvery's existing identity ("React for modern terminal apps. Powerful apps. Polished UIs.") and **add** Ink compatibility as a feature, not the headline. Working candidate: subhead mentions "Compatible with Ink — keep your code if you need," and "Migrating from Ink" appears as a third-tier nav item, not the primary CTA. The verified claims (98.6% Ink compat, 3-27× faster than Ink, 1.5-5.5× faster than Yoga via flexily) **stay on the page** — they're already battle-tested and on silvery.dev today. The correction is voice, not claims: don't *lead* the hero with "Drop-in for Ink" or use the migration framing as the primary identity (too direct, risks Ink-community backlash). Keep the perf/compat numbers in a Performance section / feature row — just don't make them the leading wedge.
...
14. **Loud launch push** — only after demo + 1 case study + 1 positive maintainer signal in hand. Blog post leading with silvery's capabilities (multi-pane, mouse, streaming, performance) rather than anti-Ink framing. Conference angle softened from *"Beyond Ink: a new rendering contract"* (still implies succession) to *"Silvery: React for AI-era TUIs"* — silvery on its own merits, Ink-compat as one capability among several. HN, Reddit, JS Weekly, X/Twitter, conference CFPs. Outreach to maintainers framed as "we built something you might find useful," not "we built the migration path off your platform."
...
17. ~~Migration Assurance Program v0~~ — **reactive only, not promoted**. Decision 2026-04-28: don't publicly launch a sponsored-migration program with SLAs and committed slots. If a maintainer reaches out asking for migration help, say yes case-by-case (sponsored with co-authored write-up, or paid for larger jobs). No public landing page, no Calendly slot, no public SLA. Reasons: (a) shipping a public services offering at framework launch dilutes the OSS brand voice toward consultancy; (b) we don't want to commit engineering capacity to slot-fulfillment that competes with codemod / case study / demo work; (c) if the codemod + PR bot + create-silvery CLI work, formal MAP-as-program isn't needed — the migration becomes self-serve. Keep MAP on the menu as a fallback offer; don't build infra around it until inbound demand justifies it.
