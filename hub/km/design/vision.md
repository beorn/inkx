...
**Namespaces — sigils mark namespaces, not types.** Any node can declare itself a namespace via the `namespace` facet. A namespace node has a sigil (`#`, `@`, `!`, or none), a name, and a minting strategy for child IDs. Children of the namespace get short IDs formed as `{sigil}{name}^{local}` — e.g., `#silvery^47` for item 47 in the `#silvery` room namespace, `@alice^1` for item 1 in `@alice`'s namespace, `TUI^47` for area-prefixed beads with no sigil. The `^` separator disambiguates compound names (`#silvery-refactor` is a compound room name; `#silvery^refactor` is local ID `refactor` in the `#silvery` namespace) and matches Obsidian's block-reference convention. Sigil conventions (`#` for rooms, `@` for people/agents) are inherited from the namespace declaration, not hardcoded as type markers. Tracked in `km-infra.namespaces`.
