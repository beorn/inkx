...
```tsx
// Don't do this — simplified to show the pattern
interface Message {
  id: number
  content: string
}

// Assume these come from your app's state management:
// stream: AsyncIterable<string> — the LLM token stream
// setMessages: React state setter for Message[]
// activeId: number — the message being streamed into
...
// simplified — setMessages and stream come from your app context
const pending = { current: "" }
const BATCH_MS = 16 // one frame at 60fps

function startBatching(messageId: number, setMessages: React.Dispatch<React.SetStateAction<Message[]>>) {
...
At 16ms intervals, you get at most 62 renders per second regardless of token rate. Each render handles whatever tokens accumulated since the last batch. The visual result still feels continuous -- humans can't perceive text changes faster than about 15 updates per second anyway.
...
As of now, Claude Code runs inline (native scrollback). Completed exchanges scroll up into the terminal's history. You can scroll back to the beginning of the conversation with your terminal's native scroll mechanism. This feels natural, but it means it's much harder to update earlier output -- no collapsing tool call results after the fact, no updating a progress bar in a previous message.
...
// simplified — InputPrompt and ExchangeView are your own components
