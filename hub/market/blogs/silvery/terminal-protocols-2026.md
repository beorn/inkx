...
**Support.** Ghostty, Kitty, iTerm2, WezTerm, and Warp all support it -- five of the six terminals in this comparison. Terminal.app does not. tmux has partial support (passthrough).
...
**Support.** This is widely adopted. All six terminals in this comparison -- Ghostty, Kitty, iTerm2, WezTerm, Warp, and Terminal.app -- support it. tmux passes it through. Among the terminals I've tested on [terminfo.dev](https://terminfo.dev), I haven't found a modern terminal that doesn't support it.
...
**Support.** All six terminals in this comparison support clipboard writing. Clipboard reading is more restricted for security reasons -- an arbitrary application reading your clipboard silently is a real concern. iTerm2 allows clipboard reading by default; most others require explicit user opt-in.
...
**Support.** All six terminals in this comparison support OSC 8. It's also supported by tmux (since 3.4), VS Code's terminal, and most Linux terminals.
...
**Kitty graphics protocol** is newer and more capable. Images are transmitted as PNG or RGB data, support transparency, can be placed at arbitrary positions, and can even be animated. Transmission uses escape sequences that carry the image payload inline, but the rendered image is displayed separately from the text flow.
...
**For application developers.** If you need inline images, you'll likely need to support multiple protocols. Detect the terminal (or better, probe for capability) and choose accordingly. For the broadest support, sixel gets you xterm, Terminal.app, and WezTerm; Kitty graphics gets you Ghostty, Kitty, Warp, and Terminal.app; OSC 1337 gets you iTerm2.

## Focus Reporting, Truecolor, Semantic Prompts

Three protocols that are now supported across all six terminals in this comparison:
...
**Truecolor** (24-bit RGB via `\x1b[38;2;R;G;Bm`) gives applications full-spectrum color. All six terminals support truecolor.
...
| Protocol            | Ghostty | Kitty | iTerm2 | WezTerm | Warp | Terminal.app |
| ------------------- | ------- | ----- | ------ | ------- | ---- | ------------ |
| Kitty keyboard      | Yes     | Yes   | Yes    | Yes     | Yes  | No           |
| Synchronized output | Yes     | Yes   | Yes    | Yes     | Yes  | Yes          |
| OSC 52 write        | Yes     | Yes   | Yes    | Yes     | Yes  | Yes          |
| OSC 52 read         | No      | No    | Yes    | No      | No   | No           |
| OSC 8 hyperlinks    | Yes     | Yes   | Yes    | Yes     | Yes  | Yes          |
| Kitty graphics      | Yes     | Yes   | No     | No      | Yes  | Yes          |
| Sixel               | No      | No    | No     | Yes     | No   | Yes          |
| Focus reporting     | Yes     | Yes   | Yes    | Yes     | Yes  | Yes          |
| Truecolor           | Yes     | Yes   | Yes    | Yes     | Yes  | Yes          |
| Semantic prompts    | Yes     | Yes   | Yes    | Yes     | Yes  | Yes          |
