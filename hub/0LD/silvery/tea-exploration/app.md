...
The app kernel is `create()` -- see [era2-overview.md](../../reference/era2-overview.md) § Part 0:
...
`withApp()` is a **composition preset** that combines `withCommands()`, model registry, and optionally provider wiring into a single convenience plugin. `withScope()` is a separate plugin in the pipe -- `withApp()` does not include it. It is not a standalone package export -- it's what the `silvery` bundle provides, or what users write themselves by composing the underlying packages (`@silvery/commands`, `@silvery/scope`, `@silvery/model`).
...
See [era2-overview.md](../../reference/era2-overview.md) § Part 2 for the full `withApp()` implementation. The key points:
...
- **`app.command()`** -- typed dispatch convenience (returns `op.pending`), resolves refs via tree walk
...
`when()` gates keybindings by a signal predicate evaluated at input time. Domain plugins that reference other domains' commands must come after them in the pipe. See [era2-overview.md](../../reference/era2-overview.md) § when() for the full API, and the `withEditor()` example for cross-domain binding composition.
...
Providers are the app's I/O capabilities — persistence, AI, network, terminal. They live on `app.providers` and flow into models as typed dependencies.

### Why providers aren't just plugins
...
Models declare their provider dependencies via `Pick<typeof providers, "persist" | "ai">`, keeping the dependency surface explicit and testable. See [signals.md](./signals.md) § Model (via `createModel`) for how model factories receive providers.
...
```typescript
const chat = chatModel.create({ persist: mockPersist, ai: mockAI })
chat.submit({ text: "hello" })
await chat.save()
expect(mockPersist.written).toContain("chat.json")
```

This is the same `.create(mockDeps)` pattern described in [signals.md](./signals.md) § Testing. Each test gets isolated state and isolated I/O.
