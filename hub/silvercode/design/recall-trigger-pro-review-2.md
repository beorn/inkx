...
3) Prevent “amplifying past mistakes”
...
4) Cost ceiling estimate (claude-haiku-4-5 ~ $0.01/call)
- Assumptions:
...
5) What production systems use for the “valid query” router
...
2. **The Fast Planner:** If `probe: true`, pass the `focus` to your existing FTS planner to generate the 10-20 search variants.
3. **FTS + Rerank:** Run the parallel FTS queries (600ms).
4. **Metadata Framing (No Synth):** Format the top 3-5 hits with their Bead Status (`[RESOLVED]`, `[ABANDONED]`) and timestamps.
5. **Async Injection:** If this pipeline takes < 2.5s, block the prompt and prepend it. If it takes longer, fire the agent immediately, run the recall in parallel, and inject the framed hits as an ambient "System Observation" mid-turn between the agent's tool calls. 
