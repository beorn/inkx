...
- **gbrain** (Garry Tan, v0.9.1, https://github.com/garrytan/gbrain) — closest validation of the four-tier framework. Ships 3 of 4 tiers in production for personal-life corpus: Tier 1 (`gbrain search` tsvector + `gbrain query` hybrid), Tier 2 ("brain-first lookup on every message" agent discipline), Tier 4 (nightly "dream cycle" cron — entity sweep + citation fixes + consolidation). Lacks Tier 3 (no continuous reactive sub-agent watching events). Validates "compiled truth + timeline" format, multi-source event ingestion (voice/email/X/calendar), and operational cron+recipes shape. Different corpus (personal life), same architecture. silvercode + tribe could host gbrain as a P3 sub-agent — different corpus, same framework. See [`recall-thought.md`](../../../tribe/design/recall-thought.md) for full mapping.
