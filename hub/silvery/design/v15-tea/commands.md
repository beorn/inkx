...
This is good for AI (see [AI Mode](../v-undecided/ai-mode.md)), but it's good for _everyone_. The properties that make an app AI-native — self-describing, programmatically drivable, structured state — also make it more testable, more accessible, more composable, and self-documenting.
...
It centralizes arg defaults, signal-based availability, and validation. Every surface calls the same function and then handles the result according to its nature. See [era2-overview.md](../../reference/era2-overview.md) § resolveInvocation() for the normative behavior table.
...
`when()` returns per-binding descriptors carrying a live signal accessor. The signal is called at input time -- lazy evaluation, not reactive subscription. Focus is a signal condition -- `when(focusModel.hasFocus, { ... })`. See [era2-overview.md](../../reference/era2-overview.md) § when() for the full API and type signatures.
...
Each command definition has a **universal core** (title, description, fn, args) and optional **surface-specific overrides** (custom CLI flags, menu position). Keybindings and modes live on keymap bindings, not command definitions. One definition, not separate definitions in separate files. The tree structure groups commands into **domain objects** — typed namespaces that are navigable rather than flat. An AI exploring the app sees 6 domain objects, not 173 undifferentiated commands (see [AI Mode: Discovery](../v-undecided/ai-mode.md#discovery-domain-objects--types)).
...
Plugins compose the tree: a chat plugin defines `commands.chat.submit` and `commands.chat.compact`, a history plugin adds `commands.history.undo` and `commands.history.redo`. Commands are plain objects on the tree — structural placement IS registration. No separate step needed. The **ref-to-path mapping** for serialization (`app.commands.todo.add` → the string `"todo.add"` for CLI/MCP/replay/logging) is derived by walking `app.commands` via `findPathInTree()`. Invoking a command has negligible overhead (it's the same `fn()` the UI calls, not IPC or serialization).
