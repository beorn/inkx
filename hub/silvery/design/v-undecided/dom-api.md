...
```tsx
import { render, createTerm } from "@silvery/ag-term"
...
### Core: `silvery.render()` as universal entry point
...
interface SilveryRoot {
  ...
  createRoot(locator: AutoLocator): SilveryRoot
}
...
```tsx
import silvery from "@silvery/ag-term"

// Auto-creates term, enters alternate screen, handles cleanup
using root = await silvery.createRoot()
root.render(<App />)
...
```tsx
await silvery.run(<App />)
```

#### Pattern 2: Static/headless render

```tsx
const root = silvery.createRoot({ cols: 80, rows: 24 })
...
### Phase 1: Unified SilveryRoot interface (non-breaking)

Add `silvery.createRoot()` as a new entry point alongside existing APIs. It wraps the existing `SilveryInstance` internally but returns the unified `SilveryRoot` interface. No existing code needs to change.

```typescript
// New entry point (src/silvery.ts)
export const silvery = {
  async createRoot(options?: RootOptions): Promise<SilveryRoot> {
    // Internally: ensureLayoutEngine() + createTerm() + new SilveryInstance()
    // Returns SilveryRoot wrapping the instance + App capabilities
  },
  async run(element: ReactElement, options?: RootOptions): Promise<void> {
    using root = await silvery.createRoot(options)
    root.render(element)
    await root.waitUntilExit()
  },
}
...
The `SilveryRoot` interface combines:
...
Implementation: `SilveryRoot` wraps `SilveryInstance` and delegates to a `buildApp()` internally for locator/text/screenshot functionality.

### Phase 3: Nested mounting

Add `createRoot(locator)` to `SilveryRoot`. This requires:
...
Once `silvery.createRoot()` is stable and the runtime layers use it internally:
...
- Update CLAUDE.md and docs to show `silvery.createRoot()` as the primary API

## Implementation Notes

### SilveryRoot internal structure

```typescript
class SilveryRootImpl implements SilveryRoot {
  private instance: SilveryInstance // Terminal management, lifecycle
  private app: App // Locators, text, buffer access
  private children: Map<string, SilveryRootImpl> // Nested roots by locator selector
  ...
  createRoot(locator: AutoLocator): SilveryRoot {
    ...
    const child = new SilveryRootImpl({
      cols: bbox.width,
      rows: bbox.height,
