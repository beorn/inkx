...
A React context + hook that manages a Map of `(sectionIndex, itemIndex) -> ScreenRect`. Items auto-register via `useScreenRect` on mount and auto-unregister on unmount.
...
Wraps an item and automatically registers its screen position. Minimal overhead — just a Box with `useScreenRect` + `useEffect` cleanup.
