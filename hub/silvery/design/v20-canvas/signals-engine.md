...
## Proven pattern: km's alien-trees

km already has a production signal-on-tree system in `alien-trees` (`alien-trees` (vendor/bearly/packages/alien-trees/src/index.ts)):
...
```typescript
// Hypothetical layout-as-signals (same alien-trees DSL pattern)
...
The key: alien-trees already handles the hard parts (lazy node creation, tree rebinding, efficient walk caching). Extending it to layout signals is a natural progression, not a new invention.
...
**alien-trees overhead**: km's alien-trees lazily creates per-node signal records. For a 500-node kanban, ~2000 signals (4 per node). Cost is dominated by alien-signals' internal tracking, which benchmarks at ~50ns per computed update. 2000 × 50ns = 100μs for a full-tree recompute — well under 1ms.
...
**Mitigation**: Spatial index (grid/hash) as a computed signal. `cell(5,3) = computed(() => topmost(spatialIndex.at(5,3)))`. When a node moves, only affected index entries invalidate. Same pattern as km's `rectRegistry` for hit testing. Can also use the alien-trees `.reduce()` pattern: `cellValue: tree.descendants(s => s.renderedCells).reduce(overlay, emptyCell)` — the overlay reducer takes the topmost non-transparent cell. Proven in production with km's `excludedSigils` aggregate.
...
- **Granular invalidation**: Instead of invalidating everything on rebind, only invalidate computeds whose traversal path includes the mutated edge. A new sibling of node X doesn't affect X's descendants — only X's parent's children-aggregates. The alien-trees DSL knows direction (up/down) and can scope invalidation.
...
| Overlapping regions   | Medium            | **Solved**       | Spatial index + alien-trees reduce       |
...
**P4 — keep as speculative exploration.** Terminal doesn't need it (2.5-5.2x wins are sufficient). Canvas v2.0 might — evaluate after @silvery/solid ships and we have real canvas performance data. The alien-trees pattern is proven for tree aggregates (km uses it in production); extending to layout is a natural next step IF canvas profiling shows the tree-walk pipeline as a bottleneck.

**First concrete step**: When canvas v2.0 profiling shows pipeline as bottleneck, prototype layout-as-signals using the alien-trees DSL pattern on a 10K-element canvas scene. Compare with current flexily tree walk. If signals win by 5x+, proceed with full engine.
