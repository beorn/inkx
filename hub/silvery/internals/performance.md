...
For raw benchmark numbers, reproduce with `bun run bench`. For the head-to-head Ink comparison with context and code examples, see [Silvery vs Ink](../../silvery/docs/guide/silvery-vs-ink.md).
...
3. **Render phase is fast at 1.7us** -- incremental clone + dirty skip. Output phase (7.5us for no-change, 45us for 10% changes) is the bottleneck for the diff path.
...
| --- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.1 | **Granular dirty flags**        | Separate `contentDirty`, `layoutDirty`, `stylePropsDirty` -- style-only changes skip layout, content changes skip paint. Props compared before marking dirty. |
...
| #   | Optimization               | Description                                                                                                                                                                                                                                                                |
| --- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4.1 | **Incremental rendering**  | Clone previous buffer; only re-render dirty subtrees. 7-flag fast-path skip (`contentDirty`, `stylePropsDirty`, `layoutChangedThisFrame`, `subtreeDirty`, `childrenDirty`, `childPositionChanged`, `hasPrevBuffer`). Includes skipBgFill and scroll viewport clear gating. |
| 4.2 | **layoutChangedThisFrame** | Authoritative per-frame flag set by `propagateLayout`, cleared by render phase. Replaces stale `!rectEqual(prevLayout, boxRect)` which was permanently true when layout phase skipped. Reduces render phase from O(N) to O(dirty) on no-layout-change frames.              |
...
```typescript
import { measurePhase, layoutPhase, renderPhase, outputPhase } from "@silvery/ag-term/pipeline"
