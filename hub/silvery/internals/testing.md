...
import { Text, renderToString } from '@silvery/ag-term';
...
```bash
# Create test harness that aliases silvery → ink
mkdir -p node_modules/ink
echo 'export * from "@silvery/ag-term";' > node_modules/ink/index.js
...
```bash
# Compare silvery output vs Ink output side-by-side
bun run visual-diff tests/fixtures/complex-layout.tsx

┌─────────────────────────────────────┬─────────────────────────────────────┐
│ Ink (reference)                     │ silvery (current)                      │
├─────────────────────────────────────┼─────────────────────────────────────┤
│ ┌────────────────────────────────┐  │ ┌────────────────────────────────┐  │
│ │ Header                         │  │ │ Header                         │  │
│ ├────────────────────────────────┤  │ ├────────────────────────────────┤  │
...
import { render as silveryRender } from '@silvery/ag-term';
import { ComplexLayout } from './fixtures/complex-layout';

group('Initial render', () => {
  bench('Ink', () => inkRender(<ComplexLayout />));
  bench('silvery', () => silveryRender(<ComplexLayout />));
});

group('Re-render (state change)', () => {
  // Setup: render once, then benchmark updates
  const { rerender } = silveryRender(<ComplexLayout count={0} />);

  bench('silvery rerender', () => {
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  silvery Compatibility Dashboard                                       v0.1.│  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
...
│  ═══════════════════════════════════════════════════════════════════════    │
│                                                                             │
│  Initial Render    [█████████░░░░░░░░░░░]  0.9x  (silvery: 4.5ms, Ink: 5ms) │ │
│  Re-render         [████████████████████]  1.2x  (silvery: 1.5ms, Ink: 1.8ms│ │
│  Memory            [██████████████░░░░░░]  1.1x  (silvery: 11MB, Ink: 10MB) │ │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
describe('Memory management', () => {
  ...
  test('useBoxRect subscriptions cleaned up', async () => {
    function LayoutUser() {
...
describe('No visual flicker', () => {
  test('useBoxRect components render without flicker', async () => {
    const frames: string[] = [];
