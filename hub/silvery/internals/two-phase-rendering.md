...
│  ├── Hooks: useBoxRect(), useScreenRect()                    │
...
└──────────────────────────────────────────────────────────────────┘
                       ...
                       RenderAdapter interface
                                ↓
┌──────────────────────┐  ┌──────────────────────┐  ┌──────────────────────┐
│   @silvery/ag-term   │  │  │   @silvery/canvas    │  │   @silvery/native    │
...
4. **Layout Hooks** - `useBoxRect()`, `useScreenRect()` implementation
...
```typescript
import { createCanvasAdapter, renderToCanvas } from '@silvery/ag-term/canvas';
...
```typescript
import { createDOMAdapter, renderToDOM } from '@silvery/ag-term/dom';
...
└── Output: Tree of SilveryNodes with layout nodes + callbacks
...
1. **Know their size** - `useBoxRect()` returns actual dimensions
...
A critical design constraint: components that adjust their size based on `boxRect` could oscillate infinitely.
...
```
@silvery/core          (~60% of code)
├── React reconciler
├── Layout engine interface
├── Two-phase pipeline
├── Hooks (useBoxRect, etc.)
└── Style system

@silvery/ag-term          (current, production)
├── TerminalBuffer (character cells)
├── ANSI code generation
├── Unicode/cell width handling
└── stdin event parsing

@silvery/canvas        (future)
├── Canvas 2D / WebGL buffer
├── Pixel-based text measurement
├── DOM event → silvery event translation
└── requestAnimationFrame integration

@silvery/native        (future, high value)
...
- **Risk**: Components adjusting size based on boxRect could oscillate
