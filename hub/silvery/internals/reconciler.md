...
const hostConfig: HostConfig = {
  ...
  commitUpdate(node, updatePayload, type, oldProps, newProps) {
    // Single-pass classification — one iteration over the union of old/new
    // keys, returning layoutChanged + contentChanged in one result. Replaces
    // the older 3-pass approach (propsEqual + layoutPropsChanged +
    // contentPropsChanged) — see packages/ag-react/src/reconciler/helpers.ts.
    const { layoutChanged, contentChanged } = classifyPropChanges(oldProps, newProps)
...
```typescript
function renderPhase(root: SilveryNode): TerminalBuffer {
...
function useSilveryNode(): SilveryNode {
  ...
  if (!node) throw new Error("useBoxRect must be used within silvery")
  return node
}

function useBoxRect(): ComputedLayout {
...
The render phase skips hidden nodes:
