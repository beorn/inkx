...
Each `Message` component can call `useBoxRect()` to know exactly how wide it is and truncate or wrap accordingly. The framework handles overflow measurement, scroll position, and viewport clipping. No height estimation required.
...
- How to import components (`import { Box, Text } from "@silvery/ag-term"`)
...
- How to debug issues (`SILVERY_STRICT=1`, `DEBUG=silvery:*`)
...
Phase 2: LAYOUT
  ...
  dimensions for every node. useBoxRect() subscribers
...
Real numbers on Apple M1 Max, Bun 1.3.9 (February 2026). Reproducible via `bun run bench:compare`. See [performance.md](../internals/performance.md) for current numbers and the full optimization catalog.
...
Silvery is API-compatible with Ink. Same `Box`, `Text`, `useInput`, `useApp`, `Static`, `Spacer` components. Same hook signatures. If your app works with Ink, it works with Silvery with minimal changes:
...
// After (silvery)
import { render, Box, Text, useApp } from "@silvery/ag-term"
import { useInput } from "@silvery/ag-term/runtime"
```

What you gain:

- `useBoxRect()` and `useScreenRect()` -- layout feedback during render
...
Silvery includes a headless renderer with Playwright-inspired locators:
...
```bash
bun add @silvery/ag-term react flexily
```
...
Silvery is **experimental** -- actively developed and used in production (it powers [km](https://github.com/beorn/km), a terminal workspace for knowledge workers), but APIs may change. The terminal render target is stable. Canvas and DOM render targets are prototypes.
...
If you're building terminal UIs for AI tools and you want components that know their size, scrollable containers that handle variable-length output, and a command system that AI agents can introspect -- give Silvery a look.
