...
Silvery's default design system is **[Sterling](/guide/sterling)** — Primer grammar, Material vocabulary, preservative derivation from terminal color schemes. It ships with ~50 semantic tokens and typography presets. 84 color schemes (Catppuccin, Nord, Dracula, Tokyo Night, Solarized, …) work automatically via OSC 10/11 auto-detection.
...
```tsx
// Sterling tokens — adapt to any theme automatically
...
<Em>aside</Em>               // italic
<Small>fine print</Small>    // $fg-muted (pre-dimmed hex at truecolor)
<Link>clickable</Link>       // $fg-accent + underline (composed by preset)
...
Hardcoded colors marry one theme. Raw SGR marries whatever that specific terminal does with SGR 2 and SGR 1. **Sterling tokens and typography presets marry them all.**
:::

→ [Styling guide](/guide/styling) · [Sterling](/guide/sterling) · [Token Taxonomy](/guide/token-taxonomy) · [Themes gallery](/themes)
...
6. **Style with Sterling tokens** — `$fg-accent` / `$bg-surface-subtle`, not hardcoded values
