...
- **Relevance to era2**: Silvery's reactivity contract is `Signal<T>` — `{ (): T, subscribe(fn): () => void }`. The callable accessor `() => T` for reading plus `.subscribe()` for reactive consumption. Compatible with alien-signals, SolidJS, and Angular signals natively. Era2 could adopt TC39 signals as the interop contract while using a faster implementation internally.
...
**Era2**: `createStore()` with deep proxy tracking via [alien-deepsignals](https://github.com/CCherry07/alien-deepsignals). Property access at any depth returns callable accessors: `user.address.city()` to read, `user.address.city("LA")` to write. Decision 27. See [signals.md](../design/v15-tea/signals.md).
...
**Era2**: `createResource(async () => fetcher())` bridges async into the signal graph. Returns callable accessor for data plus `.loading()` and `.error()` sub-accessors. Scope-integrated: parent dispose aborts in-flight fetches. Decision 28. See [signals.md](../design/v15-tea/signals.md).
...
- `signal()`, `computed()`, `batch()` — same shape as TC39, Preact, Solid, Angular
- `Signal<T>` contract (`{ (): T, subscribe() }`) — framework-agnostic, compatible with alien-signals, SolidJS, Angular
...
The App layer depends on **`Signal<T>`** — `{ (): T, subscribe(fn): () => void }` — not on any signal package. `@silvery/signals` is the default implementation, but alien-signals, SolidJS, and Angular signals all satisfy this interface natively. Preact signals adapt via thin wrappers. This contract is simpler and more stable than TC39 signals (which use `.get()` not `()`). TC39 signals would need a wrapper.
...
**Recommendation**: Don't depend on TC39 signals (Stage 1, may stall). Keep `Signal<T>` as silvery's contract — `{ (): T, subscribe() }`. Ship `@silvery/signals` (alien-signals) as default. The renderer stays signal-agnostic. Commands use the weaker `() => boolean` contract (no subscription needed).
...
| Store + atom bridge           | Per-atom (O(1))     | jotai-zustand createAtomicStore (PR #8) | ~200-400 bytes              |
...
- **`Signal<T>` contract** (`{ (): T, subscribe() }`) — signal-agnostic reactivity (renderer has zero signal deps)
