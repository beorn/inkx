...
playground/
  ...
  src/
    ...
    evaluate.ts       -- Safe eval with React + silvery in scope
...
- Auto-completion for `<Box>`, `<Text>`, `useBoxRect()`, etc.
...
function Editor({ value, onChange }) {
  ...
  useEffect(() => {
    // Load silvery type definitions for intellisense
    monaco.languages.typescript.typescriptDefaults.addExtraLib(silveryTypeDefs, "silvery.d.ts")
  }, [])
...
import { Box, Text, useBoxRect, useScreenRect } from "@silvery/ag-exp-canvas"

const scope = { React, Box, Text, useBoxRect, useScreenRect }
...
const debouncedRender = useMemo(
  () =>
    debounce((code: string) => {
      ...
      if (Comp) {
        instance.rerender(<Comp />) // silvery canvas render
      }
    }, 150),
  [instance],
)
...
```
https://stackblitz.com/github/user/silvery-playground
```
...
```
https://silvery-playground.example.com/#code=ZnVuY3Rpb24gQXBwKCkg...
```
