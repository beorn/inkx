...
> Note: The run(app, fn) pattern evolved into app.run() as a method on the app object. See era2-overview.md Part 0 — renderer provides run(), adapter wraps it.
...
> Note: The .get() API is outdated. Per Decision 29, signals use callable accessors (count() to read, count(5) to write). Selectors call accessors explicitly: useModel(todoModel, m => m.cursor()). See era2-overview.md Models section.
...
> Note: Refined by era2-overview.md. run() is not a standalone function — renderer provides app.run(), adapter wraps it, and withScope wraps run() for root scope disposal. No default withTerminal(); rendering is opt-in via withTerm().
...
> Note: The .value (data) description is outdated. Per Decision 29, createResource() returns a callable accessor: profile() to read data, profile.loading() for loading state. See era2-overview.md headless test examples.
