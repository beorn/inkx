...
_See also: [commands.md](../era2b/commands.md) (command shapes, availability), [app.md](../era2b/app.md) (plugin composition, structured concurrency)._
...
No registration, no cleanup -- break or cancel stops it. The loop IS the scope: when it ends, no dangling listeners. The async iterable terminates when the scope's `AbortSignal` fires. See [app.md](../era2b/app.md) for structured concurrency details.
...
`withReact()` bridges React to the ag node tree and handles unmatched input. It wraps `apply()` to fan out `input:*` ops to `useInput()` hooks registered by components. See [00-architecture.md](./00-architecture.md) § Adapter for the full implementation.

Key detail: `prevApply(op)` is called first. If the keymap (installed by `withApp()`) handles the op, `useInput()` hooks never see it. Only unmatched input falls through to components.
...
import { create, pipe } from "@silvery/create"
import { withScope } from "@silvery/scope"
import { withApp } from "silvery"
import { when } from "@silvery/commands"
import { withAg } from "@silvery/ag"
import { withTerm } from "@silvery/ag-term"
...
`when()` returns per-binding descriptors carrying a live signal accessor. The signal is called at input time -- not at registration time. See [00-architecture.md](./00-architecture.md) § when() for the full API and type signatures.

Two separate concerns: **can the command run?** (args schema -- `resolveInvocation()` returns `"unavailable"` if signal defaults are nullish) vs **should this key trigger it?** (`when()` predicates, mode-specific). A command like `remove` is always available; the key `dd` only triggers it in normal mode. Commands don't know about modes -- that's the keymap's job. See [00-architecture.md](./00-architecture.md) § when() for the full type signature and implementation.
...
See [00-architecture.md](./00-architecture.md) § resolveInvocation() for the normative behavior table.
