...
_See also: [00-architecture.md](./00-architecture.md) (THE reference), [composability.md](../era2a/composability.md) (tradeoffs, gap analysis)._
...
Ag (rendering) and app-level packages are independent groups under one `@silvery/*` scope. Rendering uses `ag-*` prefix grouping. App-level packages use descriptive names (`model`, `commands`, `signals`, `scope`, `create`).
...
@silvery/headless          SelectListState, TextInputState, VirtualListState,
  ...
  API shape (pure state, no rendering dependency):
    const list = new SelectListState({ items, onSelect })
    list.moveDown()           // pure state mutation
    list.selected             // current selection
    list.visibleItems         // windowed subset

    const input = new TextInputState({ onChange })
    input.insert("hello")     // pure state mutation
...
Without prefixes, `@silvery/ag-react` and `@silvery/commands/react` look hierarchical — as if commands/react depends on @silvery/ag-react. It doesn't. With prefixes, they're clearly parallel:
...
- Provides React-specific hooks: `useBoxRect()`, `useFocus()` (platform-specific hooks like `useTerminalFocused()` live in `@silvery/ag-term`)
...
See [composability.md](../era2a/composability.md) for the full gap analysis and tradeoff discussion.
