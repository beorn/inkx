  ...
  return (app) => {
    ...
    app.apply = (op) => {
      if (op.type === "input:key") {
        ...
        // trie lookup, timeout, and ambiguity handling — see era2/01-rendering-input.
        ...
        for (let i = bindings.length - 1; i >= 0; i--) {
          ...
          const path = findPathInTree(app.commands, b.command)
          if (!path) continue
          const resolution = resolveInvocation(app, b.command, b.args)
          if (resolution.state === "ready") {
            queueMicrotask(() => app.dispatch({ type: "command", path, args: resolution.args }))
          } else if (resolution.state === "prompt") {
            queueMicrotask(() => app.dispatch({ type: "prompt", command: path, missing: resolution.missing }))
          }
    ...
    app.command = (refOrPath, args) => {
      const path = typeof refOrPath === "string" ? refOrPath.split(".") : findPathInTree(app.commands, refOrPath)
...
**alien-signals is the reactive engine.** `@silvery/signals` re-exports alien-signals core (`signal`, `computed`, `effect`, `batch`) and adds `createStore()` (deep proxy), `createResource()` (async bridge), and `/react` bindings (`useSignal`). Decision 26-28 in [decisions.md](../refs/decisions.md). See [signals-landscape.md](../refs/signals-landscape.md) for the full research and rationale.
...
**Command ref identity via tree walk.** `findPathInTree(app.commands, ref)` derives paths from the command tree. No explicit registration — assigning to `app.commands.todo = { ... }` is discovery. `app.command()` uses tree walk for ref → path resolution.
