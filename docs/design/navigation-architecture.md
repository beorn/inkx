...
- **Source of truth**: Column's `cardNodes` array (from `ColumnView`) -- MIXED
- **Data flow**:
...
4. **Page jump uses col.cardNodes**. This is a ColumnView-derived list, not a ViewTree walk. It only includes top-level cards (not subitems), which is intentional for page-sized jumps, but the data source is inconsistent with block/outline nav.
