...
**Status**: Deprecated - migrate to silvery `createRenderer()`.

**When reviewing Playwright tests**:

- Consider migrating to silvery (faster, more reliable)
- Keep only if testing pixel-perfect terminal rendering that silvery can't cover
- For keyboard navigation, use silvery + `stdin.write()` instead
