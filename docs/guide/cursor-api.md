...
function Editor({ lines }: { lines: string[] }) {
  const { x, y } = useBoxRectDangerously()
...
The key insight is that `useBoxRectDangerously()` provides _absolute_ terminal coordinates via `x` and `y`. This allows cursor positioning relative to a component:

```tsx
function PositionedInput() {
  const { x, y, width } = useBoxRectDangerously()
...
import { Box, Text, useFocusable, useCursor, useInput, useBoxRectDangerously } from "silvery"
...
- [useBoxRectDangerously](/api/use-box-rect) - Get component dimensions and position
