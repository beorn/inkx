...
| `cursor`                 | 2    | Hardware-cursor emission check. Replays the exact cursor-bearing output bytes through xterm.js and asserts the terminal cursor lands on the expected row/column (or is hidden). Records the last cursor frame via `getLastOutputCursorDiagnostics()` and logs to `silvery:cursor`. Use for live-path cursor bugs where the buffer is correct but the real caret parks in padding, stale transcript content, or another row after render. **Tier 2** because it creates an emulator for each checked cursor write; isolate with `SILVERY_STRICT=cursor`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
