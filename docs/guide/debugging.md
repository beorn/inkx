...
| `island-host-frame`         | 2    | The pane-host's frame (the Island's layout rect) diverges from the guest size-owner for two consecutive renders — the host re-framed but the guest never acknowledged (`requestResize` not called, or the size subscription never fired). One frame of divergence is the legal resize-ack window; a persistent divergence is the pane-host/guest desync that produces residual cells (the 20625 herdr-Island residue class, same family as the 19406 swallowed same-size SIGWINCH). Checked in `assertIslandRenderInvariants` per render. |
