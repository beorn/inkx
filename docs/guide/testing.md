...
| `hooks.test.tsx`                | 28    | useBoxRectDangerously, useFocusable, useFocusManager, useStdout       |
...
```tsx
import { useBoxRectDangerously, NodeContext } from "silvery"

function LayoutCapture({ onLayout }: { onLayout: (l: any) => void }) {
  const layout = useBoxRectDangerously()
