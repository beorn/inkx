...
- Silvery has built-in containment for `useBoxRectDangerously` that detects and breaks oscillation loops.
- If you see oscillation, check for circular dependencies in layout — e.g., a component that changes its size based on `useBoxRectDangerously` in a way that triggers another layout.
- Avoid setting `width` or `height` dynamically based on `useBoxRectDangerously` of the same Box.
...
### useBoxRectDangerously returning zeros

`useBoxRectDangerously()` returns `{ width: 0, height: 0 }` on the first render because layout hasn't run yet. This is by design.
...
- **Oscillation**: If the component changes its own size based on `useBoxRectDangerously`, it can loop. Silvery's built-in containment detects this and breaks the cycle automatically.
