...
`useBoxRectDangerously()` returns `{ width: 0, height: 0 }` on the first render (before layout runs). Guard with `if (width === 0) return null` if your component can't render without dimensions. The framework handles the re-render automatically.
:::
...
| Responsive columns   | `useBoxRectDangerously()` + conditional rendering            |
| Adaptive content     | `useBoxRectDangerously()` + truncation / hide at breakpoints |
...
Run `bunx @silvery/examples text layout` for a side-by-side visual comparison of snug-content bubbles, even wrapping, and the combined effect.
:::
