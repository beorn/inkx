...
| `showRootGuide` | `boolean`                                        | `false`      | Include branch/last guides on root rows         |
...
Roots have no guide by default. Descendants use continuation, branch, and
last-child guides. Set `showRootGuide` when a consumer's established output
includes a rail on root rows. With `guideStyle="ascii"`, guides use `|`, `|--`,
and `` `-- ``.
