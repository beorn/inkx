...
Multi-line text input with word wrapping, scrolling, selection, and cursor movement. Uses `useBoxRectDangerously` for width-aware word wrapping and scroll tracking to keep the cursor visible. Built on the `useTextArea` hook.
