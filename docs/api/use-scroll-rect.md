...
Like [`useBoxRectDangerously`](/api/use-box-rect), this hook returns the **committed** rect: the value as of the most recent event-batch commit boundary, invariant across every convergence pass within a batch.
...
Same shape as [`useBoxRectDangerously`](/api/use-box-rect): `{ width, height, x, y }`.
...
For the actual paint position on the terminal screen (sticky-clamped), use [`useScreenRect`](/api/use-screen-rect). For responsive sizing inside a component, use [`useBoxRectDangerously`](/api/use-box-rect).

## See also

- [`useBoxRectDangerously`](/api/use-box-rect) — layout position (border-box minus padding/border)
