...
Like [`useBoxRectDangerously`](/api/use-box-rect), this hook returns the **committed** rect: invariant across every convergence pass within a batch, advancing only at the next batch's commit boundary.
...
For the layout-pass position before sticky clamping, use [`useScrollRect`](/api/use-scroll-rect). For the inner content rect, use [`useBoxRectDangerously`](/api/use-box-rect).
