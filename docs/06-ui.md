...
interface BoardState {
  selectedNodeId: string | null;  // Which node is selected (stable across zoom)
  cursor: TPath;                  // View position [col, card, ...] (derived from selectedNodeId)
...
**Key design:** `selectedNodeId` is the single source of truth for which node is selected.
The `cursor` path is derived from `selectedNodeId` relative to the current `rootId`.
This means zoom operations ('u'/'Enter') change only the view, not the selection.
