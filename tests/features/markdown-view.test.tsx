...
describe("MarkdownView — block elements", () => {
  ...
  test("nested list renders sub-items indented under their parent (UL/OL/LI presets)", () => {
    const app = render("- Parent item\n  - Child one\n  - Child two")
    expect(app.text).toContain("Parent item")
    expect(app.text).toContain("Child one")
    expect(app.text).toContain("Child two")
    // The child bullet differs from the parent's — LI derives it from the shared
    // list-context depth, proving the sub-list nested rather than flattened.
    expect(app.text).toContain("◦")
    // Children sit on their own rows below the parent, not merged into it.
    const lines = app.text.split("\n").filter((line) => line.trim() !== "")
    const parentRow = lines.findIndex((line) => line.includes("Parent item"))
    const childRow = lines.findIndex((line) => line.includes("Child one"))
    expect(parentRow).toBeGreaterThanOrEqual(0)
    expect(childRow).toBeGreaterThan(parentRow)
  })
