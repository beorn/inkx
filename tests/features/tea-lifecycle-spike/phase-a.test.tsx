/**
 * @failure Reconciler mount/focus cleanup corrupts real terminal input routing,
...
// (see runtime/index.ts). Deriving it from the public run() function keeps
// this feature test off package-private implementation paths.
...
describe("Phase A — real React/Ink mount-unmount + key lifecycle", () => {
  ...
  test("A2 — Ctrl+P opens, printable keys go to dialog, Escape closes, `j` routes back to board", async () => {
    ...
    // from the screen here — this spike only asserts LIFECYCLE (dialog
    // CLOSED state, keys route to the board afterwards; checked below).
    // HISTORY: when written (2026-04), the incremental output phase left
    // the old dialog panel's cells painted after conditional-unmount
    // (tracked as @si/render/12378-stale-pixels). That class is fixed and
    // gated in vendor/silvery as of 2026-07: see
    // tests/features/modal-dismiss-no-ghost.test.tsx (buffer + termless
    // layers) and tests/regressions/tree-swap-stale-border.test.tsx. If
    // you rerun this spike and still see stale dialog cells, that is a
    // regression of those gates, not an expected artifact.
