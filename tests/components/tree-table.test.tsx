...
describe("TreeTable", () => {
  ...
  test("optionally preserves root rails for consumers whose tree owns the root row", () => {
    const render = createRenderer({ cols: 48, rows: 12 })
    const app = render(
      <TreeTable
        data={TREE}
    ...
    expect(app.lines.find((line) => line.includes("alpha"))).toContain("├── alpha")
    expect(app.lines.find((line) => line.includes("omega"))).toContain("└── omega")
  })
  ...
  test("keeps nested rows visible when custom React cells would otherwise wrap", () => {
    const customColumns = [
      {
        header: "NAME",
        grow: true,
        render: (row: Row) => <Text color="$fg-accent">{row.name}</Text>,
      },
      {
        header: "STATE",
        key: "state",
    ...
    const app = render(
      <TreeTable<Row>
        data={[
          {
            name: "root-with-long-name",
            state: "ONE1",
            children: [
              {
                name: "child-with-long-name",
                state: "TWO2",
                children: [{ name: "deep-with-long-name", state: "THR3" }],
              },
            ],
          },
        ]}
