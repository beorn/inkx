...
The core engine is operational: parser, query system, bidirectional sync, SQLite storage, CLI, and a functional TUI with kanban board view. Active development is focused on TUI polish and the rendering engine (hightea).
...
- **Mouse infrastructure**: hit registry via hightea layout feedback (position → element mapping)
...
| Area | Epic | Focus |
|------|------|-------|
| **hightea** | `km-hightea` | Terminal rendering engine — performance, correctness, incremental updates |
