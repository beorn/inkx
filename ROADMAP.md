...
The core engine is operational: parser, query system, bidirectional sync, SQLite storage, CLI, and a functional TUI with kanban board view. Active development is focused on TUI polish and the rendering engine (inkx).
...
- **Mouse infrastructure**: hit registry via inkx layout feedback (position → element mapping)
...
*Designed in [docs/future/agents.md](docs/future/agents.md).*
...
*Designed in [docs/future/services.md](docs/future/services.md).*
...
| Area | Epic | Focus |
|------|------|-------|
| **inkx** | `km-inkx` | Terminal rendering engine — performance, correctness, incremental updates |
| **flexx** | `km-flexx` | Flexbox layout engine for terminal UI |
