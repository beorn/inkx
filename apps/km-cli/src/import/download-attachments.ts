      ...
      batch.map(async (att) => {
        ...
        try {
          ...
          if (res.status === 403 && opts.refreshUrl && att.sourceId) {
            console.log(
              term.dim(`  Refreshing download URL for ${att.name}...`),
            );
          ...
          if (!res.ok) {
            console.log(
              term.yellow(
                `  Failed to download ${att.name}: HTTP ${res.status}`,
              ),
            );
            result.failed++;
            bar.increment();
          ...
          console.log(
            term.yellow(
              `  Failed to download ${att.name}: ${(err as Error).message}`,
            ),
          );
          result.failed++;
        }
