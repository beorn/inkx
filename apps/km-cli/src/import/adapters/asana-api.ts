...
async function fetchAndSaveTaskList(
  ...
  const { importSections, looseItems } = groupBySections(tasks, enrichedMap, sections, spec.projectGid)
...
function countItems(items: ImportItem[]): { tasks: number; subtasks: number; comments: number; attachments: number } {
  let tasks = 0
  let subtasks = 0
  let comments = 0
  let attachments = 0
...
function partitionFiles(downloadDir: string): { projectFiles: string[]; userFiles: string[]; tagFiles: string[] } {
