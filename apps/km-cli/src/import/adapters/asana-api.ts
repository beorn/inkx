...
export {
  resolveAsanaWorkspace,
  listAsanaStructure,
  validateAsanaToken,
} from "./asana-discovery.ts";
export {
  isSystemAction,
  filterSystemComment,
  fetchComments,
  type FetchCommentsResult,
} from "./comment-filter.ts";
export {
  toImportItem,
  enrichItem,
  fetchSubtasks,
  fetchAttachments,
} from "./task-transform.ts";
...
async function fetchProjectStatuses(
  ...
  const statuses = await client.getAll<{
    ...
    opt_fields:
      "title,text,color,author.name,created_at,modified_at",
  });
...
async function fetchCustomFieldSettings(
  client: AsanaClient,
  projectGid: string,
): Promise<ImportCustomFieldDef[]> {
  ...
  return settings
    ...
    .map((s) => {
      ...
      if (cf.enum_options?.length) {
        def.enumOptions = cf.enum_options
          .map((o) => o.name)
...
async function batchEnrich(
  ...
  for (let i = 0; i < tasks.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(
      batch.map((t) => enrichItem(client, t, enrichOpts)),
    );
...
function deriveAssigneeSections(
  tasks: AsanaApiTask[],
): Array<{ gid: string; name: string }> {
  const seen = new Set<string>();
  const result: Array<{ gid: string; name: string }> = [];
...
function groupBySections(
  ...
  for (const section of sections) {
    ...
    const items = sectionTasks
      .map((t) => enrichedMap.get(t.gid))
      .filter((item): item is ImportItem => !!item);
    if (items.length > 0) {
      importSections.push({
...
async function fetchAndSaveTaskList(
  ...
  writeFileSync(
    join(downloadDir, `${spec.slug}.json`),
    JSON.stringify(importProject, null, 2),
    "utf-8",
  );
...
function countProjectItems(proj: ImportProject): number {
  const allItems = [
    ...(proj.items ?? []),
    ...(proj.sections ?? []).flatMap((s) => s.items),
  ];
...
function partitionFiles(downloadDir: string): {
  ...
  const allFiles = readdirSync(downloadDir).filter(
    (f) => f.endsWith(".json") && !f.startsWith("_"),
  );
  return {
    projectFiles: allFiles.filter(
...
export async function fetchFromAsana(
  options: FetchOptions & { record: true },
): Promise<FetchResult>;
export async function fetchFromAsana(
...
export async function fetchFromAsana(
  ...
  if (options.workspace) {
    workspace =
      me.workspaces.find(
        (w) => w.name === options.workspace || w.gid === options.workspace,
      ) ?? firstWorkspace;
  } else {
  ...
  const existingFiles = readdirSync(downloadDir).filter(
    (f) => f.endsWith(".json") && !f.startsWith("_"),
  );
  let emptyOnDisk = 0;
  for (const f of existingFiles) {
    const proj = JSON.parse(
      readFileSync(join(downloadDir, f), "utf-8"),
    ) as ImportProject;
    const hasItems =
      (proj.items?.length ?? 0) > 0 ||
      (proj.sections ?? []).some((s) => s.items.length > 0);
    if (hasItems) {
  ...
  if (alreadyFetched.size > 0) {
    console.log(
      term.cyan(`  Resuming:`),
      `${alreadyFetched.size} project(s) already downloaded`,
    );
  }
  if (emptyOnDisk > 0) {
    console.log(
      term.yellow(`  Re-fetching:`),
      `${emptyOnDisk} empty project(s) on disk`,
    );
  }
  ...
  if (alreadyFetched.size > 0) {
    ...
    for (const p of filteredProjects) {
      ...
      if (
        ...
        p.modified_at &&
        diskProj.modifiedAt &&
        p.modified_at > diskProj.modifiedAt
      ) {
        stale.push(p.name);
      }
    }
    if (missing.length > 0) {
      console.log(
        term.cyan(`  Missing:`),
        `${missing.length} project(s) not yet downloaded`,
      );
    ...
    if (stale.length > 0) {
      console.log(
        term.yellow(`  Changed:`),
        `${stale.length} project(s) modified since last fetch`,
      );
      for (const name of stale) console.log(term.yellow(`    ~ ${name}`));
    }
    if (missing.length === 0 && stale.length === 0) {
      console.log(
        term.green(`  All ${alreadyFetched.size} project(s) up to date`),
      );
    }
  }
  ...
  for (const project of filteredProjects) {
    ...
    const { project: importProject, tasks } = await fetchAndSaveTaskList(
      client,
      {
        ...
        fetchSections: () =>
          client.get<Array<{ gid: string; name: string }>>(
  ...
  if (options.includeUserTaskLists) {
    ...
    console.log(
      term.dim(`  Fetching My Tasks for ${userList.length} user(s)...`),
    );
    ...
    for (const user of userList) {
      ...
      try {
        taskList = await client.get<{ gid: string }>(
        ...
        console.log(
          term.dim(`  @${userSlug}: skipped (task list not accessible)`),
        );
        continue;
      }
      ...
      const orphanTasks = allUserTasks.filter(
        (t) => !capturedTaskGids.has(t.gid),
      );

      if (orphanTasks.length === 0) {
        console.log(
          term.dim(
            `  @${userSlug}: 0 orphan tasks (${allUserTasks.length} total, all in projects)`,
          ),
        );
        continue;
      }
      ...
      console.log(
        term.cyan(
          `  @${userSlug}: ${orphanTasks.length} orphan tasks (${allUserTasks.length} total)`,
        ),
      );
  ...
  if (options.includeTagTaskLists) {
    ...
    for (const tag of tags) {
      ...
      const orphanTasks = allTagTasks.filter(
        (t) => !capturedTaskGids.has(t.gid),
      );
      ...
      if (orphanTasks.length === 0) {
        console.log(
          term.dim(
            `  #${tagSlug}: 0 orphan tasks (${allTagTasks.length} total, all in projects)`,
          ),
        );
        continue;
      }
      ...
      console.log(
        term.cyan(
          `  #${tagSlug}: ${orphanTasks.length} orphan tasks (${allTagTasks.length} total)`,
        ),
      );
  ...
  if (options.record && client.recorded.length > 0) {
    writeFileSync(
      join(downloadDir, "_recording.json"),
      JSON.stringify(client.recorded, null, 2),
      "utf-8",
    );
  }
...
function verify(
  ...
  for (const f of [...projectFiles, ...userFiles, ...tagFiles]) {
    ...
    try {
      ...
      diskTasks += diskCount;
      const memProj = importData.projects.find(
        (p) => p.sourceId === proj.sourceId,
      );
      if (memProj) {
        const memCount = countProjectItems(memProj);
        if (memCount !== diskCount) {
          mismatches.push(
            `${proj.title}: memory=${memCount} disk=${diskCount}`,
          );
        }
      }
    } catch {
  ...
  const fileBreakdown = [
    ...
    .join(" + ");
  console.log(
    `  Projects: ${importData.projects.length} in memory, ${diskProjects} on disk (${fileBreakdown} files)`,
  );
  console.log(
    `  Tasks: ${memTasks} top-level, ${memSubtasks} subtasks (${memTasks + memSubtasks} total)`,
  );
  ...
  console.log(
    `  Disk: ${(diskBytes / 1024 / 1024).toFixed(1)} MB across ${totalFiles} JSON files`,
  );

  if (diskTasks !== memTasks + memSubtasks) {
    console.log(
      term.yellow(
        `  \u26a0 Task count mismatch: memory=${memTasks + memSubtasks}, disk=${diskTasks}`,
      ),
    );
  } else {
  ...
  if (emptyProjects > 0) {
    const empties = importData.projects.filter(
      (p) => !(p.items?.length || p.sections?.length),
    );
    console.log(
      term.yellow(
        `  \u26a0 ${emptyProjects} empty project(s): ${empties.map((p) => p.title).join(", ")}`,
      ),
    );
  }
  ...
  for (const [gid, expected] of expectedTaskCounts) {
    ...
    const actual =
      (proj.items?.length ?? 0) +
      (proj.sections ?? []).reduce((n, s) => n + s.items.length, 0);
    if (actual !== expected) {
      mismatches.push(
        `${proj.title}: API returned ${expected} tasks, stored ${actual}`,
      );
    }
  }
  ...
  if (importData.projects.length !== diskProjects) {
    mismatches.push(
      `Project count: memory=${importData.projects.length}, disk=${diskProjects}`,
    );
  }
