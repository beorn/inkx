...
export async function resolveAsanaWorkspace(
  ...
  if (workspaceFilter) {
    const found = workspaces.find(
      (w) => w.name === workspaceFilter || w.gid === workspaceFilter,
    );
    if (!found) {
      throw new Error(
    ...
    console.log(
      ...
      term.dim(
        `Use --workspace to select: ${workspaces.map((w) => w.name).join(", ")}`,
      ),
    );
  }
...
export async function listAsanaStructure(
  ...
  for (const ws of workspaces) {
    ...
    const [allRaw, users, tags] = await Promise.all([
      ...
      client.getAll<{ gid: string; name: string }>(
        `/workspaces/${ws.gid}/users`,
        {
          opt_fields: "name",
          limit: "100",
        },
...
export async function validateAsanaToken(
  ...
  const res = await fetch(
    `${ASANA_BASE}/users/me?opt_fields=name,email,workspaces.name`,
    {
  ...
  if (res.status === 401) {
    throw new Error(
      "Invalid token. Get a new one at https://app.asana.com/0/developer-console",
    );
  }
  if (!res.ok) {
