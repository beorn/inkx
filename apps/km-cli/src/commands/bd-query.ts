 ...
 *
 * Bead-centric default: prepends `fstype:mdfile` to the DSL unless the
 * user already mentions `fstype` in their expression. Beads ARE mdfiles
 * (one bead = one .md file); inline list-item checkboxes inside a bead
 * body are acceptance criteria, not separate beads. `--all-tasks` opts
 * out for the rare case where the user wants to query checkboxes too.
 * For the un-opinionated raw surface, use `km query` directly.
 */
...
export function registerBdQuery(parent: BdRegistrar): void {
  const queryCmd = new Command("query")
    ...
    .description("Query nodes with raw DSL (alias for `km query`; bd-style: defaults to fstype:mdfile)")
    .option("--all-tasks", "Don't add the default fstype:mdfile filter — include inline-checkbox sub-tasks too")
