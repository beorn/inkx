/**
 * Pure planner for `task orphans` (and the `bd orphans` thin shim that
 * delegates here).
 *
 * Lifted from `bd-orphans-plan.ts` as part of Wave 6 of
 * `@km/cli/task-bd-collapse` — find open beads/tasks referenced in
 * recent commit messages (i.e. work implemented but not formally
 * closed).
 *
 ...
 *
 * Acceptance contract:
 *   - Match the canonical id as a whole word (no `\b` since `.` isn't
 ...
 * canonical id. Returns only issues with at least one matching commit
 * (i.e., the orphans).
 */
export function findOrphans<I extends IssueLike>(issues: readonly I[], commits: readonly CommitEntry[]): Orphan<I>[] {
  ...
  for (const issue of issues) {
    // Match the canonical id as a whole word — `\b` doesn't treat `.`
