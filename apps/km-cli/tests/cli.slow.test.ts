...
describe("km view - state initialization", () => {
  ...
  // (starts with /, ./, ../, ~/) and doesn't exist on disk, throwing a
  // CliError that index.ts renders as the canonical `error: Vault not found:`
  // format — same shape km show / km list use via loadRepo(). Bare ids /
  ...
  test("km view with absolute non-existent path exits 1 with helpful error", async () => {
    ...
    expect(out).toMatch(new RegExp(`Vault not found: ${missing.replace(/\//g, "\\/")}`))
  ...
  test("km view with relative ./missing path exits 1", async () => {
    ...
    expect(result.exitCode).toBe(1)
    const out = result.stderr + result.stdout
    expect(out).toMatch(/Vault not found: \.\/km-test-definitely-missing-xyz789/)
  })
  ...
  test("km view with ~/missing tilde path exits 1", async () => {
    ...
    expect(result.exitCode).toBe(1)
    const out = result.stderr + result.stdout
    expect(out).toMatch(/Vault not found:/)
  })
  ...
  test("km view does NOT fail-fast on bare ids (no path prefix)", async () => {
    ...
    // pre-flight "Vault not found" error.
    const result = await km(["view", "@nonexistent-bead-xyz"])
    const out = result.stderr + result.stdout
    expect(out).not.toMatch(/Vault not found:/)
  })
})
