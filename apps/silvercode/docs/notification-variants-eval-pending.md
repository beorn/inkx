# Notification Variants Eval — Pending Run

**Status:** harness shipped, results pending user run.
**Date queued:** 2026-04-27
**Bead:** [`km-silvercode.notification-split-test`](bd-show:km-silvercode.notification-split-test)
**Driver:** [`apps/silvercode/tests/eval/notification-variants.eval.ts`](../tests/eval/notification-variants.eval.ts)
...
NOTIFICATION_SPLIT_MODELS=claude-sonnet-4-6 \
NOTIFICATION_SPLIT_TRIALS=25 \
  bun apps/silvercode/tests/eval/notification-variants.eval.ts
```

The real run writes `apps/silvercode/docs/notification-variants-eval-<YYYY-MM-DD>.md` with per-cell tables, per-scenario decisions, and ratio computations. Once that file lands, this `-pending.md` stub can be deleted (the bead closes against the dated file).
...
If results are noisy: re-run with `NOTIFICATION_SPLIT_TRIALS=100` and re-check. Stochasticity at low rates is the usual culprit.
