 ...
 *   2. Notification events batched until next prompt — channelQueue → notificationStream
 ...
 * wire.ts / notification-stream.ts / channel-queue.ts breaks one of these.
 */
...
// ===========================================================================
// Bug 2 — Notification events arrive incrementally (not batched)
// ===========================================================================

describe("bug 2 — notification events stream incrementally per record", () => {
  test("channelQueue.subscribe fires synchronously per enqueue (no batching)", async () => {
    await using scope = createScope("test-notification-incremental")
  ...
  test("notificationStream.record fires subscribers per event, in order, with fresh entries snapshot", async () => {
    ...
    await using scope = createScope("test-notification-fanout")
  ...
  test("queue → stream pipeline fans out incrementally — no event held until turn boundary", async () => {
    ...
    // notificationStream.record. Verifies no intermediate buffering.
    await using scope = createScope("test-notification-pipeline")
    const queue = createChannelQueue(scope)
    const stream = createNotificationStream(scope)
...
describe("bug 3 — failed tool emits single tool_call_update, no duplicate error event", () => {
  test("tool-result with is_error:true produces exactly one tool_call_update SessionUpdate", async () => {
    ...
    // a separate "Error" row in the UI on top of the failed ToolCall block.
    //
