/**
 * Ambient wire-bytes verification — Phase 2 of
 ...
 * text on the wire (e.g. concatenate ambient bodies into a single text
 ...
 * # Why these payloads
 *
 * Sample ambient content reads "peer alice opened PR #42" and "CI passed"
 * — benign factual strings deliberately containing zero role-prefix
 * trigger tokens (per the content-quarantine discipline in
 * `ambient-context-safety.md` §9). The user text is "continue", the
 ...
 *
 * Bead: `km-silvercode.ambient-phase-2-adapter-wire`.
 */
...
import { assembleAcpPrompt, ambientUri, AMBIENT_FRAMING_PREFIX } from "../../../src/prompt-assembly.ts"
...
const AMBIENT_BODY = "peer alice opened PR #42"
const USER_TEXT = "continue"

function ambientEvent(): ChannelEvent {
  return {
    id: "evt-001",
    source: "tribe",
    timestamp: 1_700_000_000_000,
    content: AMBIENT_BODY,
  } as ChannelEvent
}
...
describe("ambient wire-bytes — every backend routes ambient as resource ContentBlock, never as user text", () => {
  for (const id of REGISTRY_IDS) {
    test(`${id}: assembleAcpPrompt → agent.prompt wire shape`, async () => {
      ...
      await using scope = createScope(`test-ambient-wire-${id}`)
      ...
      // assembleAcpPrompt with autoInject so the queued ambient event is
      // routed as a typed EmbeddedResource block, not flattened into text.
      const queue = makeQueueWith([ambientEvent()])
      ...
      const ambientBlock = wire[0]!
      const userBlock = wire[1]!

      // 1. Ambient lands in `type: "resource"` (EmbeddedResource), not text.
      expect(ambientBlock.type).toBe("resource")
      if (ambientBlock.type !== "resource") throw new Error("unreachable")
      // The resource carries `text` (TextResourceContents) with the framing
      // prefix prepended. URI is the canonical ambient:// scheme.
      const resource = ambientBlock.resource
      expect(resource.uri).toBe(ambientUri("tribe", "evt-001"))
      expect("text" in resource ? resource.text : "").toContain(AMBIENT_BODY)
      expect("text" in resource ? resource.text : "").toContain(AMBIENT_FRAMING_PREFIX)
      ...
      // 3. Ambient payload does NOT appear in the user-text block. This is
      //    the load-bearing assertion — if any adapter flattened ambient
      //    into role-U text, AMBIENT_BODY would show up here.
      expect(userBlock.text).not.toContain(AMBIENT_BODY)
      expect(userBlock.text).not.toContain(AMBIENT_FRAMING_PREFIX)

      // 4. No other text block on the wire carries the ambient payload —
      //    even if the wire shape were extended with extra blocks later, the
      //    invariant "ambient bytes never inhabit a text ContentBlock" must
      //    hold.
      for (const block of wire) {
        if (block.type === "text") {
          expect(block.text).not.toContain(AMBIENT_BODY)
        }
      }
    })
  }
...
// ---------------------------------------------------------------------------
// Sanity: the boundary itself routes ambient correctly even with no autoInject
...
describe("ambient wire-bytes — boundary invariants", () => {
  test("autoInject:false → only user text on the wire (no ambient leak)", async () => {
    const { spawn, capture } = createCapturingAcpServer()
    __setAcpSpawnForTesting(spawn)

    await using scope = createScope("test-ambient-wire-no-inject")
    ...
    if (wire[0]!.type === "text") {
      ...
      expect(wire[0]!.text).not.toContain(AMBIENT_BODY)
    }
  })

  test("multiple ambient events all route as resource, none flatten to text", async () => {
    const { spawn, capture } = createCapturingAcpServer()
    __setAcpSpawnForTesting(spawn)

    await using scope = createScope("test-ambient-wire-multi")
    const session = await connectAcpRegistry(scope, "gemini", { cwd: "/tmp/wire-test" })

    const events: ChannelEvent[] = [
      { id: "e1", source: "tribe", timestamp: 1, content: AMBIENT_BODY } as ChannelEvent,
    ...
    if (wire[2]!.type === "text") {
      ...
      expect(wire[2]!.text).not.toContain(AMBIENT_BODY)
