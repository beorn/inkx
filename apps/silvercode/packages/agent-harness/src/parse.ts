...
export function createStreamJsonParser(emit: Emit): StreamJsonParser {
  ...
  function handleUserEcho(obj: Record<string, unknown>): void {
    ...
    // commands" banner). These aren't real user input and shouldn't
    // appear in the resumed transcript. Live stream-json never sets
    // isMeta, so this branch only matters for replay.
    ...
    if (typeof content === "string") text = normalizeUserText(content)
