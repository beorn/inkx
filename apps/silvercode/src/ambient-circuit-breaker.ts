/**
 * Ambient circuit breaker — token-bucket rate-limit for the ambient
 * pipeline (Phase 6.b of the ambient-context safety design — see
 * `hub/silvercode/design/ambient-context-safety.md` § 4 Phase 6.b and
 * § 5 Safeguards).
 *
 ...
 * adversarial CI hook) can still flood the ambient channel with
 ...
 * `ambient-telemetry.ts`) rather than catastrophic.
 *
 * Policy:
 *
 *   - Per-source bucket: 10 events/min (configurable via
 *     `SILVERCODE_AMBIENT_PER_SOURCE_PER_MIN`).
 *   - Global bucket: 50 events/hour (configurable via
 *     `SILVERCODE_AMBIENT_GLOBAL_PER_HOUR`).
...
export type AmbientBreakerOpts = {
...
export type AmbientBreakerStats = {
  readonly perSourcePerMin: number
  readonly globalPerHour: number
 ...
 */
export function createAmbientBreaker(opts: AmbientBreakerOpts = {}): AmbientBreaker {
  const perSourceCap = opts.perSourcePerMin ?? readPositiveIntEnv("SILVERCODE_AMBIENT_PER_SOURCE_PER_MIN", 10)
  const globalCap = opts.globalPerHour ?? readPositiveIntEnv("SILVERCODE_AMBIENT_GLOBAL_PER_HOUR", 50)
  ...
  // Telemetry counters — internal, separate from `ambient-telemetry.ts`'s
