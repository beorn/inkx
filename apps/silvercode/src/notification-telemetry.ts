/**
 * Notification Layer 4 telemetry — observability surface for the
 * notification-context safety stack (`apps/silvercode/docs/channels.md`
 ...
 *     marker in a notification payload or assistant turn.
 ...
 *
 * Loggily namespace: `silvercode:notification`. Each `record*` helper emits a
...
const log = createLogger("silvercode:notification")
...
/**
 * One notification telemetry counter snapshot. Returned by `getTelemetrySnapshot`.
 ...
 *
 *   - `sanitize` — Layer 2 (notification payload, before resource construction).
 ...
 *
 * `source` is the notification source tag (`tribe`, `recall`, …) for Layer 2
...
/** Kind of Layer 2 transformation that sanitizeNotification applied. */
...
export function recordSanitizeAction(args: { source: string; action: SanitizeAction; sessionId?: string }): void {
  ...
  log.info?.("notificationSanitized", {
    source: args.source,
    action: args.action,
    sessionId: args.sessionId,
  })
...
export function recordDropped(args: { source: string; reason: DropReason; sessionId?: string }): void {
  ...
  log.warn?.("notificationEventDropped", {
    source: args.source,
    reason: args.reason,
    sessionId: args.sessionId,
  })
 ...
 * (`notification`, `actionable`, …) — today this is always `"notification"`, but
...
export function recordAdmitted(args: { source: string; kind: string; sessionId?: string }): void {
  ...
  log.info?.("notificationEventAdmitted", {
    source: args.source,
    kind: args.kind,
    sessionId: args.sessionId,
  })
