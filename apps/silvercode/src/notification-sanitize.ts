/**
 * Notification payload sanitization (Layer 2 of the notification-context safety
 * stack — see `apps/silvercode/docs/channels.md`).
 *
 * Pure deterministic function, called on every notification payload before
 ...
 *   4. Size-bound (`MAX_NOTIFICATION_BYTES`, default 16 KiB) with a visible
 *      `[truncated …]` marker.
 *
 * Idempotent: `sanitizeNotification(sanitizeNotification(x)) === sanitizeNotification(x)`.
...
const TRUNCATION_MARKER = "\n\n[truncated — payload exceeded notification size cap]"
...
/**
 * Sanitize one notification payload. Pure, idempotent, no I/O.
 *
 * Pass order:
 ...
 */
export function sanitizeNotification(payload: string, opts: { maxBytes?: number } = {}): string {
  return sanitizeNotificationWithReport(payload, opts).output
}

/**
 * Per-action breakdown of what `sanitizeNotification` actually did to the
 * input. Returned by `sanitizeNotificationWithReport` so Layer 4 telemetry
 ...
 * pass order in `sanitizeNotification`.
 */
...
/**
 * Same as `sanitizeNotification` but reports which passes fired. Used by
 * the telemetry-instrumented call-sites; pure-logic call-sites should
 * keep using `sanitizeNotification` directly.
 */
export function sanitizeNotificationWithReport(payload: string, opts: { maxBytes?: number } = {}): SanitizeReport {
  const maxBytes = opts.maxBytes ?? MAX_NOTIFICATION_BYTES
 ...
 * `sanitizeNotification` would neutralize? Useful for telemetry — Layer 4
 * detection logs notification payloads where this returns true.
 */
