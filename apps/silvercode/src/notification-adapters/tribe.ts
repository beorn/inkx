/**
 * Tribe notification adapter — tails the bearly tribe activity log and emits
 * peer broadcasts / DMs as `source: "tribe"` notification events.
 *
 * This is Phase 6.b's *replacement* surface for the existing
 * `subscribeTribe` in `channel-sources.ts`. Differences:
 *
 *   - Sanitizes every payload through `sanitizeNotification` (Layer 2 of the
...
const dTribe = createDebug("silvercode:notification:tribe")

const SOURCE = "tribe" as const

export type TribeAdapterOptions = NotificationAdapterCtx & {
 ...
 */
export function registerTribeNotificationAdapter(opts: TribeAdapterOptions): () => void {
 ...
 */
export function emitTribeLineForTest(opts: NotificationAdapterCtx, line: string): boolean {
  const emit = createDebouncedEmit(opts)
  let parsed: TribeLine
  try {
    parsed = JSON.parse(line) as TribeLine
