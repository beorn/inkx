/**
 * Notification adapter types — Phase 6.b of the notification-context-excellence epic.
 *
 * An adapter is a small, scope-bound subscriber that:
 *
 *   1. Watches one notification signal source (tribe broadcast, fs change, CI
 *      verdict, recall hit, sub-agent completion, …).
 *   2. Sanitizes every payload through `sanitizeNotification` (Layer 2 of the
 *      notification-context safety stack — see
 ...
 *      per-session `NotificationStream` for inline UI display.
 ...
 *
 * Each adapter file exports a `register…NotificationAdapter(opts)` factory
 ...
 * `NotificationEventRow` (`SOURCE_PRESENTATION`) and the per-source mute
 ...
 * sources in sync with `NotificationEventRow.tsx`.
 */
export type NotificationSource = "tribe" | "recall" | "subagent" | "ci" | "filewatch" | "telegram"
 ...
 *
 * `now()` and `enqueue()` are wired by `registerAllNotificationAdapters`. They
 ...
 */
export type NotificationAdapterCtx = {
 ...
 *   2. `sanitizeNotification` on `content`.
 ...
 */
export function createDebouncedEmit(ctx: NotificationAdapterCtx): AdapterEmit {
  ...
  return (raw): boolean => {
    ...
    lastEmit = t
    const sanitized = sanitizeNotification(raw.content)
...
export function makeNotificationEventId(source: NotificationSource | string): string {
