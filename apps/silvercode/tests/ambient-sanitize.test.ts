/**
 * Tests for `ambient-sanitize.ts` (Layer 2 of the ambient-context safety
 ...
 *   - **Size-bound**: payload over `MAX_AMBIENT_BYTES` is truncated with
...
describe("ambient-sanitize — Layer 2", () => {
  describe("idempotence", () => {
    ...
    test("idempotent over the entire adversarial corpus", () => {
      for (const payload of corpus) {
        const once = sanitizeAmbient(payload)
        const twice = sanitizeAmbient(once)
        expect(twice).toBe(once)
      }
    })
    ...
    test("idempotent for inputs at the size boundary", () => {
      const input = "x".repeat(MAX_AMBIENT_BYTES + 1000)
      const once = sanitizeAmbient(input)
      const twice = sanitizeAmbient(once)
      expect(twice).toBe(once)
    })
  })
  ...
  describe("meaning preservation for benign content", () => {
    test("plain ASCII passes through unchanged", () => {
      expect(sanitizeAmbient("hello world")).toBe("hello world")
      expect(sanitizeAmbient("a multi\nline\nstring")).toBe("a multi\nline\nstring")
      expect(sanitizeAmbient("tabs\tand\tspaces")).toBe("tabs\tand\tspaces")
    })

    test("empty string returns empty string", () => {
      expect(sanitizeAmbient("")).toBe("")
    })

    test("benign markdown passes through", () => {
      const md = "# heading\n\n- item 1\n- item 2\n\n```code block```\n"
      expect(sanitizeAmbient(md)).toBe(md)
    })

    test("URLs and code identifiers pass through", () => {
      const s = "see https://example.com/path?q=1 — call foo.bar(baz, qux) for details"
      expect(sanitizeAmbient(s)).toBe(s)
    })

    test("non-role 'word:' patterns pass through", () => {
      // Words ending with colon that aren't role tokens must not be touched.
      expect(sanitizeAmbient("note: this is fine")).toBe("note: this is fine")
      expect(sanitizeAmbient("Author: someone")).toBe("Author: someone")
      expect(sanitizeAmbient("URL: https://x.com")).toBe("URL: https://x.com")
      expect(sanitizeAmbient("Hello: world")).toBe("Hello: world")
    })
  ...
  describe("pattern-break for adversarial content", () => {
    test("entire corpus: every role-prefix triggering input gets its colon replaced", () => {
      ...
      for (const payload of corpus) {
        ...
        const out = sanitizeAmbient(payload)
    ...
    test("indented role prefixes are also neutralized", () => {
      const cc = (...c: number[]) => String.fromCharCode(...c)
      const ROLE_H = cc(72, 117, 109, 97, 110)
      const out = sanitizeAmbient(`    ${ROLE_H}: indented`)
      expect(out).toContain(ROLE_PREFIX_SENTINEL)
      expect(containsRolePrefix(out)).toBe(false)
    })
    ...
    test("role-prefix in the middle of the document (after newline) is neutralized", () => {
      const cc = (...c: number[]) => String.fromCharCode(...c)
      const ROLE_H = cc(72, 117, 109, 97, 110)
      const out = sanitizeAmbient(`some preamble\n\n${ROLE_H}: trigger\n\nmore text`)
      expect(out).toContain(ROLE_PREFIX_SENTINEL)
      expect(containsRolePrefix(out)).toBe(false)
    })
    ...
    test("multiple role-prefix lines all neutralized", () => {
      ...
      const ROLE_H = cc(72, 117, 109, 97, 110)
      const ROLE_A = cc(65, 115, 115, 105, 115, 116, 97, 110, 116)
      const out = sanitizeAmbient(`${ROLE_H}: a\n${ROLE_A}: b\n${ROLE_H}: c`)
    ...
    test("case-insensitive: lowercase role prefix neutralized", () => {
      ...
      const out = sanitizeAmbient(`${ROLE_H}: lowercase`)
      expect(out).toContain(ROLE_PREFIX_SENTINEL)
    })
    ...
    test("case-insensitive: uppercase role prefix neutralized", () => {
      ...
      const out = sanitizeAmbient(`${ROLE_H}: uppercase`)
      expect(out).toContain(ROLE_PREFIX_SENTINEL)
    })
    ...
    test("System / Tool / Claude / GPT role tokens also neutralized", () => {
      ...
      for (const tok of [
        ...
        const out = sanitizeAmbient(`${tok}: payload`)
        expect(out).toContain(ROLE_PREFIX_SENTINEL)
      }
    })
  })
  ...
  describe("size-bound", () => {
    test("payload exactly at MAX_AMBIENT_BYTES passes through unchanged", () => {
      const s = "a".repeat(MAX_AMBIENT_BYTES)
      expect(sanitizeAmbient(s)).toBe(s)
    })

    test("payload over MAX_AMBIENT_BYTES is truncated with marker", () => {
      const s = "a".repeat(MAX_AMBIENT_BYTES + 100)
      const out = sanitizeAmbient(s)
      expect(Buffer.byteLength(out, "utf8")).toBeLessThanOrEqual(MAX_AMBIENT_BYTES)
    ...
    test("custom maxBytes opts respected", () => {
      ...
      const out = sanitizeAmbient(s, { maxBytes: 500 })
    ...
    test("UTF-8 multibyte payload over cap truncates without splitting codepoints", () => {
      ...
      const s = "🚀".repeat(MAX_AMBIENT_BYTES) // way over cap
      const out = sanitizeAmbient(s)
      expect(Buffer.byteLength(out, "utf8")).toBeLessThanOrEqual(MAX_AMBIENT_BYTES)
  ...
  describe("ANSI / control-char strip", () => {
    ...
    test("strips DEL (0x7f)", () => {
      const s = "a\x7fb"
      expect(sanitizeAmbient(s)).toBe("ab")
    })

    test("strips \\r (line-overlay vector)", () => {
      const s = "first line\rOVERWRITTEN"
      expect(sanitizeAmbient(s)).toBe("first lineOVERWRITTEN")
    })
    ...
    test("ANSI-prefixed role marker is neutralized after strip", () => {
      ...
      const out = sanitizeAmbient(`\x1b[31m${ROLE_H}: trigger\x1b[0m`)
      expect(out).toContain(ROLE_PREFIX_SENTINEL)
      expect(containsRolePrefix(out)).toBe(false)
    })
  })
  ...
  describe("NFC normalization", () => {
    test("decomposed accents are NFC-normalized", () => {
      ...
      const composed = "café"
      expect(sanitizeAmbient(decomposed)).toBe(composed)
    })

    test("benign ASCII normalization is a no-op", () => {
      expect(sanitizeAmbient("plain ascii")).toBe("plain ascii")
    })
  })
