/**
 * Tests for `ambient-circuit-breaker.ts` (Phase 6.b — see
...
describe("ambient-circuit-breaker", () => {
  ...
  describe("env var defaults", () => {
    const ORIG_PS = process.env.SILVERCODE_AMBIENT_PER_SOURCE_PER_MIN
    const ORIG_G = process.env.SILVERCODE_AMBIENT_GLOBAL_PER_HOUR

    beforeEach(() => {
      process.env.SILVERCODE_AMBIENT_PER_SOURCE_PER_MIN = ORIG_PS
      process.env.SILVERCODE_AMBIENT_GLOBAL_PER_HOUR = ORIG_G
    })

    test("env vars override defaults when opts not provided", () => {
      process.env.SILVERCODE_AMBIENT_PER_SOURCE_PER_MIN = "3"
      process.env.SILVERCODE_AMBIENT_GLOBAL_PER_HOUR = "100"
      const now = 0
      const breaker = createAmbientBreaker({ now: () => now })
    ...
    test("defaults applied when env vars unset / invalid", () => {
      delete process.env.SILVERCODE_AMBIENT_PER_SOURCE_PER_MIN
      delete process.env.SILVERCODE_AMBIENT_GLOBAL_PER_HOUR
      const now = 0
      const breaker = createAmbientBreaker({ now: () => now })
    ...
    test("ignores non-positive env values", () => {
      process.env.SILVERCODE_AMBIENT_PER_SOURCE_PER_MIN = "0"
      process.env.SILVERCODE_AMBIENT_GLOBAL_PER_HOUR = "-5"
      const now = 0
      const breaker = createAmbientBreaker({ now: () => now })
      expect(breaker.stats().perSourcePerMin).toBe(10)
      expect(breaker.stats().globalPerHour).toBe(50)
