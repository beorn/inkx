...
describe("autolinks popover — markdown rendering for readme / first-paragraph", () => {
  ...
  test("readme preview: bold markup renders as bold cells (not literal asterisks)", () => {
    ...
    writeFileSync(join(dir, "README.md"), "Project with **boldword** in its summary.\n")
    ...
    const app = render(<PopoverFrame match="~repo" preview="readme" resolvesTo={dir} body={result.body} />)
  ...
  test("first-paragraph preview: italics render as italic cells", () => {
    writeFileSync(join(dir, "doc.md"), "Plain text with *italicword* mixed in.\n")
    const result = resolvePreview({
    ...
    const render = createRenderer({ cols: COLS, rows: ROWS })
    const app = render(
      <PopoverFrame match="AGENTS.md" preview="first-paragraph" resolvesTo={join(dir, "doc.md")} body={result.body} />,
    )
    ...
    expect(cell.italic, `expected italic attr at (${italCol},${italRow}); cell=${JSON.stringify(cell)}`).toBe(true)
  })
})
