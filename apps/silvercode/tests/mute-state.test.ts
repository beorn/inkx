...
describe("createMuteState", () => {
  test("noisy notification sources and debug channel are muted by default", async () => {
    await using scope = createScope("mute-state-test")
    const state = createMuteState(scope)

    expect(state.isMuted("filewatch")).toBe(true)
    expect(state.isMuted("debug")).toBe(true)
    expect(state.muted()).toEqual(new Set(["filewatch", "debug"]))
  })
})
