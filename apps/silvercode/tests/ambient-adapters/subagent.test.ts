/**
 * Tests for the sub-agent ambient adapter — Phase 6.b real path.
 *
 * Covers:
...
describe("ambient-adapter/subagent", () => {
  ...
  test("notifyTaskToolResult — completed (non-error) emits completed event with truncated digest", () => {
    ...
    let t = 1_000
    const queue = createChannelQueue(scope)
    const handle = registerSubagentAmbientAdapterHandle({ scope, queue, now: () => t })
  ...
  test("sanitizeAmbient is applied — ANSI escapes stripped from result digest", () => {
    const scope = createScope("test")
    let t = 1_000
    const queue = createChannelQueue(scope)
    const handle = registerSubagentAmbientAdapterHandle({ scope, queue, now: () => t })
  ...
  test("sanitizeAmbient is applied — leading role-prefix in result digest is neutralized", () => {
    const scope = createScope("test")
    let t = 1_000
    const queue = createChannelQueue(scope)
    const handle = registerSubagentAmbientAdapterHandle({ scope, queue, now: () => t })
  ...
  test("digestMax override caps the digest length", () => {
    ...
    let t = 1_000
    const queue = createChannelQueue(scope)
    const handle = registerSubagentAmbientAdapterHandle({ scope, queue, now: () => t, digestMax: 20 })
    handle.notifyTaskToolUse({
      toolUseId: "toolu_cap",
      toolName: "Task",
