/**
 * Tests for the filewatch notification adapter — Phase 6.b.
 *
...
describe("notification-adapter/filewatch", () => {
  ...
  describe("register", () => {
    test("returns a disposer that is idempotent", () => {
      ...
      const dir = mkdtempSync(join(tmpdir(), "notification-filewatch-"))
      const dispose = registerFilewatchNotificationAdapter({ scope, queue, cwd: dir, recursive: false })
    ...
    test("survives an invalid cwd (logs, does not throw)", () => {
      const scope = createScope("test")
      const queue = createChannelQueue(scope)
      const dispose = registerFilewatchNotificationAdapter({
        scope,
        queue,
    ...
    test("emits a sanitized event when the watcher fires for a real file", async () => {
      ...
      const queue = createChannelQueue(scope)
      const dir = mkdtempSync(join(tmpdir(), "notification-filewatch-"))
      registerFilewatchNotificationAdapter({ scope, queue, cwd: dir, recursive: false })
