...
describe("notification-adapters/types", () => {
  ...
  test("emit runs payload through notification-sanitize (Layer 2)", () => {
    const scope = createScope("test")
    const queue = createChannelQueue(scope)
  ...
  test("makeNotificationEventId yields a unique source-tagged id", () => {
    const a = makeNotificationEventId("tribe")
    const b = makeNotificationEventId("tribe")
    expect(a).not.toBe(b)
