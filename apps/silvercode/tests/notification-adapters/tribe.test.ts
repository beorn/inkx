/**
 * Tests for the tribe notification adapter — Phase 6.b.
 *
 * Two layers of test:
 *
...
import { emitTribeLineForTest, registerTribeNotificationAdapter } from "../../src/notification-adapters/tribe.ts"
...
describe("notification-adapter/tribe", () => {
  test("emit-line: activity-log shape produces a sanitized peer-message event", () => {
    ...
    const line = JSON.stringify({
      ...
      // apps/silvercode/docs/channels.md. Embedding the trigger after a newline
  ...
  test("register: missing bus path yields a no-op disposer", () => {
    const scope = createScope("test")
    const queue = createChannelQueue(scope)
    const dir = mkdtempSync(join(tmpdir(), "notification-tribe-"))
    const dispose = registerTribeNotificationAdapter({
      scope,
      queue,
  ...
  test("register: appending a line to the bus enqueues an event", async () => {
    ...
    registerTribeNotificationAdapter({ scope, queue, busPath })
