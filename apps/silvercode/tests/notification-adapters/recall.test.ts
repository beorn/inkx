/**
 * Tests for the recall notification adapter — Phase 6.b.
 *
...
import {
  ...
  triggerRecallProbe,
} from "../../src/notification-adapters/recall.ts"
...
// (matches the `notification-sanitize.ts` convention — see § 9 of the
// notification-context-safety design doc).
...
describe("notification-adapter/recall", () => {
  test("default register is a no-op disposer", () => {
    const scope = createScope("test")
    const queue = createChannelQueue(scope)
    const handle = registerRecallNotificationAdapterHandle({ scope, queue })
  ...
  test("probe is a no-op after dispose", async () => {
    const scope = createScope("test")
    const queue = createChannelQueue(scope)
    const handle = registerRecallNotificationAdapterHandle({
      scope,
      queue,
  ...
  test("sanitize-at-line-start: role-prefix at line start is quarantined before enqueue", async () => {
    ...
    // in turn calls `sanitizeNotification(raw.content)`. The digest builder
    ...
    const { sanitizeNotification } = await import("../../src/notification-sanitize.ts")
    expect(sanitizeNotification(event?.content ?? "")).toBe(event?.content ?? "")
  })
