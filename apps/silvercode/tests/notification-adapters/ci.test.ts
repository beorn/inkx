/**
 * Tests for the CI notification adapter — Phase 6.b.
 *
 * The poller is exercised via `probeCiOnce` + `diffCi` (pure, easy to
 * test). The full timer-driven `registerCiNotificationAdapter` is exercised
...
import { diffCi, diffCiEvent, probeCiOnce, registerCiNotificationAdapter } from "../../src/notification-adapters/ci.ts"
...
describe("notification-adapter/ci", () => {
  ...
  describe("register", () => {
    test("returns a disposer that stops polling", async () => {
      ...
      let calls = 0
      const dispose = registerCiNotificationAdapter({
        scope,
        queue,
        cwd: "/tmp",
        pollMs: 10,
