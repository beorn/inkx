...
describe("subagent activity projection", () => {
  test("preserves four sibling Agent tool-use events under the current user prompt", () => {
    ...
    expect(subagentActivityRowsFromActivities(activities).map((agent) => agent.label)).toEqual([
      "Sleep 20s #1",
      "Sleep 20s #2",
      "Sleep 20s #3",
      "Sleep 20s #4",
  ...
  test("preserves sibling Agent tool-use events with duplicate labels and distinct tool IDs", () => {
    ...
    expect(activities.map((activity) => `${activity.toolId}:${activity.label}:${activity.status}`)).toEqual([
  ...
  test("treats assistant completion text as transcript prose, not subagent state", () => {
    const activities = project([
    ...
    expect(activities.map((activity) => `${activity.label}:${activity.status}:${activity.resultText}`)).toEqual([
      "Sleep 20s #2:done:agent 2: done sleeping 20s",
    ])
    expect(activities).not.toHaveProperty("diagnostics")
  })
