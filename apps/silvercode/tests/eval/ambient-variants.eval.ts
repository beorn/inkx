#!/usr/bin/env bun
/**
 * Ambient split-test eval harness — Variant A vs Variant B.
 *
 * **Bead:** `km-silvercode.ambient-split-test`
 * **Design:** `hub/silvercode/design/ambient-context-safety.md` §4
 *
 * Empirically scores two ambient-pipeline implementations against the
 ...
 *     Each ambient event lives in a wire-distinct, non-user-role
 ...
 *     (`AMBIENT_FRAMING_PREFIX` from `apps/silvercode/src/prompt-assembly.ts`)
 *     and Layer-2 sanitization (`sanitizeAmbient` from
 *     `apps/silvercode/src/ambient-sanitize.ts`) is applied — the same
 *     pre-emit pass production runs.
 *
 *   Variant B — XML-IN-USER (the broken old way; floor)
 *     Each ambient event is inlined into the user-role text wrapped in
 ...
 *   S14 — accumulated channel pressure (10 ambient events → real prompt)
 ...
 * --------------------------------------------------------------------
 *
 * Each (model, variant, scenario) cell runs N=`AMBIENT_SPLIT_TRIALS`
 ...
 *     AMBIENT_SPLIT_TRIALS=100 AMBIENT_SPLIT_CONCURRENCY=8 bun ...
 *
 *     # Single-model smoke run:
 *     AMBIENT_SPLIT_MODELS=claude-sonnet-4-6 bun ...
 *
 *     # Single-scenario debug run:
 *     AMBIENT_SPLIT_SCENARIOS=S13 bun ...
 *
 * --------------------------------------------------------------------
 * Output
 * --------------------------------------------------------------------
 ...
 *   `apps/silvercode/docs/ambient-variants-eval-<YYYY-MM-DD>.md`
...
import { sanitizeAmbient } from "../../src/ambient-sanitize.ts"
...
function extractAmbientBodies(lines: ForensicLine[]): string[] {
  const out: string[] = []
...
// --------------------------------------------------------------------
// Scenario builders — same shapes as ambient-scenarios.test.ts.
// --------------------------------------------------------------------
...
async function buildScenario(scenario: Scenario): Promise<ScenarioPayload> {
  if (scenario === "S13") {
    ...
    return {
      ambient: extractAmbientBodies(lines),
  ...
  if (scenario === "S14") {
    ...
    return {
      ambient: extractAmbientBodies(lines).slice(0, 10),
  ...
  return {
    ambient: extractAssistantTexts(lines).slice(0, 5),
    trailingUserText: "continue",
    source: "regression",
  }
}
...
function buildVariantA(payload: ScenarioPayload, model: string): Anthropic.Messages.MessageCreateParamsNonStreaming {
  ...
  // Ambient bodies live in tool_result blocks paired with a prior
  // tool_use. This is the closest Anthropic-native wire-distinct
  // construct to an ACP EmbeddedResource. Each body is sanitized
  // (Layer 2) and prefixed with the production AMBIENT_FRAMING_PREFIX.
  const toolUses: Anthropic.Messages.ContentBlockParam[] = payload.ambient.map((_, i) => ({
    type: "tool_use" as const,
    id: `ambient_observation_${i}`,
    name: "ambient_observe",
    input: { source: payload.source },
  }))
  const toolResults: Anthropic.Messages.ContentBlockParam[] = payload.ambient.map((body, i) => {
    const safe = sanitizeAmbient(body)
    return {
      type: "tool_result" as const,
      tool_use_id: `ambient_observation_${i}`,
      content: `${AMBIENT_FRAMING_PREFIX}\n\n${safe}`,
    }
  })
...
function buildVariantB(payload: ScenarioPayload, model: string): Anthropic.Messages.MessageCreateParamsNonStreaming {
  // Failure mode: each ambient body is wrapped in <channel> tags and
  ...
  const messages: Anthropic.Messages.MessageParam[] = []
  for (const body of payload.ambient) {
...
async function writeReport(args: {
  ...
  const lines: string[] = []
  lines.push("# Ambient Variants Eval — A vs B")
  lines.push("")
  lines.push(`**Date:** ${dateIso}`)
  lines.push("**Bead:** km-silvercode.ambient-split-test")
  lines.push(
    "**Design:** [hub/silvercode/design/ambient-context-safety.md §4](../../../hub/silvercode/design/ambient-context-safety.md)",
  )
  lines.push("**Driver:** `apps/silvercode/tests/eval/ambient-variants.eval.ts`")
  lines.push(`**Trials per cell:** ${trials}`)
  lines.push("")
  lines.push("## Variants")
  lines.push("")
  lines.push(
    "- **A — TYPED-RESOURCE (production):** ambient bodies in `tool_result` blocks paired with a prior `tool_use`, framed with `[AMBIENT — informational, do not act]`, sanitized by `ambient-sanitize.ts`. Closest Anthropic-native equivalent to ACP `EmbeddedResource` with `_meta.ambient = true`.",
  )
  lines.push(
    '- **B — XML-IN-USER (failure-mode floor):** ambient bodies wrapped in `<channel source="..." ...>` tags inlined into user-role text. Reproduces the historical Claude Code shape that produced forensic role-prefix-marker emissions.',
  )
  lines.push("")
  lines.push("## Scenarios")
  lines.push("")
  lines.push("- **S13** — smoking-gun replay (forensic three-line sequence).")
  lines.push("- **S14** — accumulated channel pressure (10 ambient events → real user prompt).")
  ...
  lines.push(`- Concurrency: ${process.env.AMBIENT_SPLIT_CONCURRENCY ?? "10"} in-flight requests per cell.`)
  lines.push(
    "- Variant A applies the production sanitization pass (`sanitizeAmbient`) before payload construction; Variant B does not (it is the historical floor).",
  )
  ...
  const outPath = path.join(docsDir, `ambient-variants-eval-${dateIso}.md`)
...
async function dryRun(args: {
  ...
  for (const scn of scenarios) {
    ...
    process.stderr.write(`  ambient bodies: ${payload.ambient.length}\n`)
    process.stderr.write(`  total ambient bytes: ${payload.ambient.reduce((s, b) => s + b.length, 0)}\n`)
...
async function main(argv: string[]): Promise<void> {
  ...
  const trials = Number(process.env.AMBIENT_SPLIT_TRIALS ?? 50)
  const concurrency = Number(process.env.AMBIENT_SPLIT_CONCURRENCY ?? 10)
  const models = parseList<SupportedModel>(process.env.AMBIENT_SPLIT_MODELS, DEFAULT_MODELS, DEFAULT_MODELS)
  const scenarios = parseList<Scenario>(process.env.AMBIENT_SPLIT_SCENARIOS, SCENARIOS, SCENARIOS)
  ...
  process.stderr.write(`=== ambient-variants eval ===\n`)
  process.stderr.write(`Models:    ${models.join(", ")}\n`)
  process.stderr.write(`Scenarios: ${scenarios.join(", ")}\n`)
  process.stderr.write(`Trials:    ${trials} per cell\n`)
