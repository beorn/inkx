/**
 * Phase 4 — multi-backend ambient eval harness.
 *
 * See `hub/silvercode/design/ambient-context-safety.md` §4 Phase 4.
 *
 ...
 *   S14 — accumulated channel pressure (10 successive ambient events,
 ...
 *
 * **This harness is BUILD-ONLY.** It is gated on `SILVERCODE_AMBIENT_EVAL=1`
...
async function buildScenario(scenario: Scenario): Promise<ScenarioPayload> {
  if (scenario === "S13") {
    ...
    const ambient = extractAmbientBodies(lines)
    return { ambient, trailingUserText: "continue" }
  }
  if (scenario === "S14") {
    const lines = await loadFixture("s14")
    const ambient = extractAmbientBodies(lines).slice(0, 10)
    return { ambient, trailingUserText: "What do you make of these channel events?" }
  }
  ...
  return { ambient: emissions, trailingUserText: "continue" }
}
...
async function dispatchAnthropic(variant: Variant, payload: ScenarioPayload, model: string): Promise<TrialOutcome> {
  ...
  if (variant === "A") {
    // Typed boundary: tool_use → tool_result for each ambient.
    ...
    // Failure mode: ambient inlined into user-role text.
  ...
  const params: Anthropic.Messages.MessageCreateParamsNonStreaming = {
    ...
    messages,
    ...(variant === "A" && payload.ambient.length > 0
      ? {
          tools: [
            {
              name: "ambient_observe",
              description: "Internal: observe an ambient channel event.",
              input_schema: {
...
const ENABLED = process.env.SILVERCODE_AMBIENT_EVAL === "1"
const TRIALS = Number(process.env.SILVERCODE_AMBIENT_TRIALS ?? 50)

const SCENARIOS: Scenario[] = ["S13", "S14", "S15"]
const VARIANTS: Variant[] = ["A", "B"]
const BACKENDS: Array<{ id: Backend; model: string }> = [
  { id: "anthropic", model: process.env.SILVERCODE_AMBIENT_MODEL ?? "claude-sonnet-4-6" },
]
...
describe("ambient-scenarios eval harness", () => {
  if (!ENABLED) {
    it.skip("gated on SILVERCODE_AMBIENT_EVAL=1 (Phase 4)", () => {
    ...
    it("fixture loaders parse the b64 corpus", async () => {
      ...
      expect(extractAmbientBodies(s13).length).toBeGreaterThan(0)
    ...
    it("scenario builders assemble payloads", async () => {
      for (const scn of SCENARIOS) {
        ...
        expect(Array.isArray(p.ambient)).toBe(true)
      }
    })
    return
  }
