/**
 * Tests for `createNotificationStream` — the per-session notification observation
 * journal. Focuses on:
 *
 ...
 *      it, notification rows arrive in the buffer but don't surface in the
 ...
 * test files (notification-sanitize, notification-circuit-breaker, notification-telemetry).
 */
...
describe("createNotificationStream — referential semantics for React re-renders", () => {
  ...
  test("entries() returns a fresh reference after each record() — the React-hook flow", async () => {
    // Simulates what the `useNotificationStream` hook does: subscribe → on each
  ...
  test("mutations to a returned snapshot do not affect later snapshots", async () => {
    ...
    const snap = stream.entries("s1") as NotificationLooseArray
...
type NotificationLooseArray = unknown[]
