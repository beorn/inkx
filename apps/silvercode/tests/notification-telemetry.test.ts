/**
 * Tests for `notification-telemetry.ts` (Layer 4 of the notification-context
 * safety stack — see `apps/silvercode/docs/channels.md`
 ...
 *   - End-to-end via `createNotificationStream`: an event flooded past the
...
import {
  ...
  resetTelemetry,
} from "../src/notification-telemetry.ts"
...
describe("notification-telemetry", () => {
  ...
  describe("counter accounting", () => {
    ...
    test("recordAdmitted bumps per-source admitted counter", () => {
      recordAdmitted({ source: "tribe", kind: "notification" })
      recordAdmitted({ source: "tribe", kind: "notification" })
      recordAdmitted({ source: "subagent", kind: "notification" })
      const snap = getTelemetrySnapshot()
  ...
  describe("end-to-end via notification-stream", () => {
    test("flood through stream → drops + admits accounted in telemetry", () => {
      ...
      const stream = createNotificationStream(scope, { breaker })
