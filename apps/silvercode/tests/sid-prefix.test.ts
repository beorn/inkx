...
describe("prefixSid", () => {
  test("applies <agent>:<sid> when no prefix present", () => {
    ...
    expect(prefixSid("claude", "abc-def")).toBe("claude:abc-def")
    expect(prefixSid("claude-code", "abc-def")).toBe("claude:abc-def")
  })

  test("idempotent — already-prefixed ids pass through unchanged", () => {
    expect(prefixSid("codex", "codex:abc123")).toBe("codex:abc123")
    expect(prefixSid("claude", "claude:xyz")).toBe("claude:xyz")
  ...
  test("does NOT recognize a different agent prefix as same agent", () => {
    ...
    expect(prefixSid("claude-code", "codex:abc")).toBe("claude:codex:abc")
  })
...
describe("parseSid", () => {
  test("extracts agent + bare sid for canonical input", () => {
    ...
    expect(parseSid("claude:uuid-here")).toEqual({ agent: "claude", bareSid: "uuid-here" })
    expect(parseSid("claude-code:uuid-here")).toEqual({ agent: "claude", bareSid: "uuid-here" })
...
describe("round-trip", () => {
  test("prefix → parse extracts the same agent + bare sid", () => {
    const cases: { agent: string; sid: string }[] = [
      ...
      { agent: "claude", sid: "uuid-1234-5678" },
