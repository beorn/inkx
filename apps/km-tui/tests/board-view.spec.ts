...
describe("Column Fold/Collapse", () => {
  ...
  test("z only affects current column", () => {
    const { board } = testEnv(() =>
      item(
        "board",
        item("col1", item("1a", item("sub1"))),
        item("col2", item("2a", item("sub2"))),
      ),
    )
    board.expect("#sub1").toExist()
    board.expect("#sub2").toExist()
...
describe("Outline Depth and Content Lines", () => {
  test("< decreases outline depth, hiding deeper children", () => {
    const { board } = testEnv(() =>
      item("board", item("col1", item("1a", item("sub1", item("deep1"))))),
    )
    board.expect("#sub1").toExist()
  ...
  test("< has minimum of 0", () => {
    const { board } = testEnv(() =>
      item("board", item("col1", item("1a", item("sub1")))),
    )
...
describe("Task Status", () => {
  test("space cycles task status through multiple states", () => {
    ...
    const { board } = testEnv(() =>
      item("board", item("col", item.task("task"))),
    )
  ...
  test("space on task does not affect other cards", () => {
    const { board } = testEnv(() =>
      item("board", item("col", item.task("task1"), item.task("task2"))),
    )
