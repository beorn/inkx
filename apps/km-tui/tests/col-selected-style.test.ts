...
describe("km-tui.col-selected-style: column selected style at column level", () => {
  ...
  it("column card area has visible yellow left border when cursor is at column level", () => {
    ...
    // Check multiple rows in the card area for a yellow left-side indicator.
    // With the fix, there should be a yellow vertical border/bar running
  ...
  it("returning to card level removes column-level yellow border", () => {
    ...
    const sepCell = board.screen.cell(sepX, separatorY)
    // When back at card level, separator should be dim
    expect(sepCell.attrs.dim, "separator should be dim when back at card level").toBeTruthy()
  })
})
