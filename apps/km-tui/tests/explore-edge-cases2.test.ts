...
describe("Exploration: Edge Cases", () => {
  test("single item board", () => {
    const { board } = testEnv(() => item("board", item("col1", item("Solo"))))
  ...
  test("wide terminal with narrow content", () => {
    const { board } = testEnv(() => item("board", item("col1", item("A"))), { columns: 200 })
    const text = board.screenshot()
    expect(text).not.toContain("[object Object]")
    expect(text).not.toContain("TypeError")
  })
  ...
  test("very narrow terminal", () => {
    const { board } = testEnv(() => item("board", item("col1", item("A"), item("B"))), { columns: 20 })
    const text = board.screenshot()
    expect(text).not.toContain("[object Object]")
    expect(text).not.toContain("TypeError")
  })
  ...
  test("tall terminal with few items", () => {
    const { board } = testEnv(() => item("board", item("col1", item("A"))), { rows: 50, columns: 80 })
    const text = board.screenshot()
    expect(text).not.toContain("[object Object]")
    expect(text).not.toContain("TypeError")
  })
  ...
  test("special characters in content", () => {
    const { board } = testEnv(() =>
      item("board", item("col1", item("<html>tag</html>"), item("path/to/file.ts"), item("key=value&other=pair"))),
    )
    const text = board.screenshot()
    expect(text).not.toContain("[object Object]")
    expect(text).not.toContain("TypeError")
  ...
  test("deeply nested tree", () => {
    const { board } = testEnv(() =>
      item("board", item("col1", item("L1", item("L2", item("L3", item("L4", item("L5", item("deep")))))))),
    )
    const text = board.screenshot()
    expect(text).not.toContain("[object Object]")
    expect(text).not.toContain("TypeError")
  ...
  test("many columns (10)", () => {
    ...
    const { board } = testEnv(() => item("board", ...cols), { columns: 200 })
    const text = board.screenshot()
    expect(text).not.toContain("[object Object]")
    expect(text).not.toContain("TypeError")
  })
