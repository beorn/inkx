  ...
  while (i < ansi.length) {
    if (ansi[i] === "\x1b") {
      if (ansi[i + 1] === "[") {
        i += 2
        let params = ""
        while (i < ansi.length && ((ansi[i]! >= "0" && ansi[i]! <= "9") || ansi[i] === ";" || ansi[i] === "?" || ansi[i] === ":")) { params += ansi[i]; i++ }
        const cmd = ansi[i]; i++
        if (cmd === "H") { if (params === "") { cx = 0; cy = 0 } else { const p = params.split(";"); cy = Math.max(0, (parseInt(p[0]!) || 1) - 1); cx = Math.max(0, (parseInt(p[1]!) || 1) - 1) } }
        else if (cmd === "K") { const n = parseInt(params) || 0; if (n === 0) for (let x = cx; x < w; x++) screen[cy]![x] = " " }
        ...
        else if (cmd === "J") { if (params === "2") for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) screen[y]![x] = " " }
      } else if (ansi[i + 1] === "]") { i += 2; while (i < ansi.length) { if (ansi[i] === "\x1b" && ansi[i + 1] === "\\") { i += 2; break }; if (ansi[i] === "\x07") { i++; break }; i++ } }
      else i += 2
    } else if (ansi[i] === "\r") { cx = 0; i++ }
    else if (ansi[i] === "\n") { cy = Math.min(h - 1, cy + 1); i++ }
    else { if (cy < h && cx < w) { screen[cy]![cx] = ansi[i]!; cx++ }; i++ }
  }
  return screen
}

interface CellStyle { char: string; fg: string; bg: string }
      ...
      if (ansi[i + 1] === "[") {
        ...
        if (cmd === "m") {
          ...
          while (j < codes.length) {
            const c = codes[j]!
            if (c === 0) { fg = "default"; bg = "default" }
            else if (c >= 30 && c <= 37) fg = `${c}`
            else if (c >= 40 && c <= 47) bg = `${c}`
            else if (c >= 90 && c <= 97) fg = `${c}`
            else if (c >= 100 && c <= 107) bg = `${c}`
            else if (c === 38 && codes[j + 1] === 5) { fg = `256:${codes[j + 2]}`; j += 2 }
            else if (c === 48 && codes[j + 1] === 5) { bg = `256:${codes[j + 2]}`; j += 2 }
        ...
        } else if (cmd === "H") { if (params === "") { cx = 0; cy = 0 } else { const p = params.split(";"); cy = Math.max(0, (parseInt(p[0]!) || 1) - 1); cx = Math.max(0, (parseInt(p[1]!) || 1) - 1) } }
        else if (cmd === "K") { const n = parseInt(params) || 0; if (n === 0) for (let x = cx; x < w; x++) { screen[cy]![x] = { char: " ", fg, bg } } }
        else if (cmd === "A") cy = Math.max(0, cy - (parseInt(params) || 1))
        else if (cmd === "B") cy = Math.min(h - 1, cy + (parseInt(params) || 1))
        else if (cmd === "C") cx = Math.min(w - 1, cx + (parseInt(params) || 1))
        else if (cmd === "D") cx = Math.max(0, cx - (parseInt(params) || 1))
        else if (cmd === "G") cx = Math.max(0, (parseInt(params) || 1) - 1)
        else if (cmd === "J") { if (params === "2") for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) screen[y]![x] = { char: " ", fg: "default", bg: "default" } }
      } else if (ansi[i + 1] === "]") { i += 2; while (i < ansi.length) { if (ansi[i] === "\x1b" && ansi[i + 1] === "\\") { i += 2; break }; if (ansi[i] === "\x07") { i++; break }; i++ } }
      else i += 2
    } else if (ansi[i] === "\r") { cx = 0; i++ }
    else if (ansi[i] === "\n") { cy = Math.min(h - 1, cy + 1); i++ }
    else { if (cy < h && cx < w) { screen[cy]![cx] = { char: ansi[i]!, fg, bg }; cx++ }; i++ }
  }
  return screen
}
