...
export function getSelectedNodes(ctx: OpCtx): KNode[] {
  ...
  if (!ctx.sel.node.cursor()) return []
  const node = ctx.repo.getNode(ctx.sel.node.cursor() as string)
  return node ? [node] : []
}
...
export function progressiveSelectAll(ctx: OpCtx): void {
  ...
  const inOutlineMode = ctx.sel.node.cursor() !== null && card !== undefined && (ctx.sel.node.cursor() as string) !== card.id
