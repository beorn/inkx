...
function navigateVertical(dir: "up" | "down", state: NavState, repo: Repo, navigator: GridNavigator): string | null {
  ...
  if (dir === "down") {
    ...
    if (isAtColumnLevel) {
      ...
      // Column header → first navigable card in column (skip index files + hidden)
...
function navigateHorizontal(
  ...
  if (colIdx < 0) {
    // Cursor is on a hidden column — redirect to nearest visible column
...
/**
 * Find the column index among the board's children (needed for stickyX/stickyY).
...
function vnColumnIndex(col: ViewNode, hiddenNodeIds?: Set<string>): number {
  ...
  let visibleIdx = 0
  for (const c of cols) {
    if (c === col) return visibleIdx
    if (!hiddenNodeIds?.has(c.id)) visibleIdx++
  }
  return -1
}
